...
const CODEX_PATH = path.resolve(
  "C:/Temple/MasterGoldenRepository/Codex_Aethon_99.json",
);
...
try {
  if (!fs.existsSync(CODEX_PATH)) {
    fs.writeFileSync(CODEX_PATH, JSON.stringify(codexPlaceholder, null, 2), {
      flag: "w",
    });
  }
} catch (err) {
...
function verifyCodexSanctity() {
  try {
    ...
    io.emit("witness-alert", {
      reason: `Codex Error: ${error.message}`,
      encrypted: true,
    });
  }
}
