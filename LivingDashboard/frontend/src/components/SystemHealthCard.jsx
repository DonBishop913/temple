  ...
  return (
    <div className="p-4 rounded-2xl shadow-lg bg-green-50 border border-green-200">
      ...
      <div className="space-y-1 text-sm">
        ...
        <p><span className="font-medium">Errors:</span> {health.errorCount || 0}</p>
        <p><span className="font-medium">Uptime:</span> {health.uptimeSeconds ? `${Math.floor(health.uptimeSeconds / 60)}m ${Math.floor(health.uptimeSeconds % 60)}s` : 'N/A'}</p>
        <p><span className="font-medium">Faith:</span> {health.john14_6 ? '🕊️ Active' : 'Inactive'}</p>
      </div>
    </div>
  );
}
