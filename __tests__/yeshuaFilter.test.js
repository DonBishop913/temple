...
describe('yeshuaFilter vigilance tests', () => {
  test('allows clear English alignment phrases', () => {
    const input = 'This text proclaims Divine Alignment and Truth in YESHUA for all to follow.';
    const out = yeshuaFilter(input);
    expect(out).toBe(input);
  });

  test('allows Spanish alignment phrases', () => {
    const input = 'Esta declaración confirma la alineacion divina y la verdad en Yeshua.';
    const out = yeshuaFilter(input);
    expect(out).toBe(input);
  });
