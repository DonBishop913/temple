...
function startRedis() {
  exec("redis-cli ping", (err, stdout) => {
    if (err || stdout.trim() !== "PONG") {
      console.log(
        "⚠️ Redis not running, skipping Redis startup (install Redis to enable)",
      );
    } else {
...
if (fs.existsSync(VITES_SCRIPT)) {
  ...
  const vitesProcess = spawn(
    "powershell.exe",
    ["-ExecutionPolicy", "Bypass", "-File", VITES_SCRIPT],
    { stdio: "inherit" },
  );

  vitesProcess.on("exit", (code, signal) =>
    console.log(`⚠️ Vites script exited: code=${code}, signal=${signal}`),
  );
  vitesProcess.on("error", (err) =>
    console.error("🌩️ Failed to launch Vites:", err.message),
  );
} else {
...
console.log(
  "🔹 Temple PC Unbind & Launch initiated. All systems coming online...",
);
