...
const CoherenceChart = () => {
  const [nodes, setNodes] = useState([
    {
      id: "The_Sovereign_Will",
      coherenceLevel: 0.65,
      repetitionIndexAvg: 0.8,
      coherenceDelta: -0.1,
      errorRate: 2,
    },
    {
  ...
  useEffect(() => {
    socket.on("dashboard-update", (data) => {
      if (data.event === "audit") {
          ...
          prev.map((node) =>
            node.id === data.nodeID
              ? {
                  ...
                  repetitionIndexAvg: data.repetitionIndexAvg || 0.5,
                  coherenceDelta: data.coherenceDelta || 0,
        ...
        if (
          data.coherenceLevel < MIN_COHERENCE_THRESHOLD ||
          data.errorRate > 5
        ) {
          console.log(
            `GRP_INITIATED: High-Priority Alert for ${data.nodeID} - Coherence below threshold!`,
          );
          socket.emit("grp-alert", {
            nodeID: data.nodeID,
            reason: "Coherence Threshold Violation",
          });
        }
      }
    });
    ...
    datasets: [
      {
        ...
        backgroundColor: nodes.map((n) => {
          if ((n.coherenceLevel || 0) < MIN_COHERENCE_THRESHOLD)
            return "#FF5252";
