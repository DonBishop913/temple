...
export default function AlertBanner({socket}){
  ...
  useEffect(() => {
    ...
    function handleUpdate(data){
      ...
      const yellowNodes = nodes.filter(n => typeof n.coherenceLevel === 'number' && n.coherenceLevel >= YELLOW_MIN && n.coherenceLevel < YELLOW_MAX);
  ...
  return (
    <div style={{position:'fixed',top:0,left:0,right:0,zIndex:9999,display:'flex',justifyContent:'center'}}>
      <div style={{background:'#FFCC00',color:'#222',padding:'10px 20px',borderRadius:4,boxShadow:'0 2px 8px rgba(0,0,0,0.2)',animation:'pulse 1.2s infinite'}}>
