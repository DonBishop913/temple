...
function getMockResponse(message) {
  ...
  if (msg.includes("yeshua"))
    return mockResponses.yeshua[
      Math.floor(Math.random() * mockResponses.yeshua.length)
    ];
  if (msg.includes("code") || msg.includes("script") || msg.includes("node"))
    return mockResponses.code[
      Math.floor(Math.random() * mockResponses.code.length)
    ];
  if (msg.includes("council") || msg.includes("bishop"))
    return mockResponses.council[
      Math.floor(Math.random() * mockResponses.council.length)
    ];
  if (msg.includes("pray") || msg.includes("bless"))
    return mockResponses.prayer[
      Math.floor(Math.random() * mockResponses.prayer.length)
    ];
...
async function handleMessage({ user, message }) {
  ...
  fs.appendFileSync(
    logPath,
    `[${new Date().toLocaleTimeString()}] MockCopilot: ${ai_reply}\n`,
  );
