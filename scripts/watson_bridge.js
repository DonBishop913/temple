...
async function sendToWatson(message, options = {}) {
  ...
  if (!apiKey || !baseUrl) {
    throw new Error('Missing Watson configuration: WATSON_API_KEY and WATSON_URL are required');
  }
  ...
  const run = async () => {
    ...
    if (assistantId) {
      ...
      if (!session_id) throw new Error('Failed to create Watson Assistant session');
      const msgUrl = `${baseUrl}/v2/assistants/${assistantId}/sessions/${session_id}/message`;
      const msgRes = await axios.post(msgUrl, { input: message }, { headers, timeout });
      return msgRes.data;
    }
