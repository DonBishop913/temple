...
const DASHBOARD_OVERLAY_FILE = path.join(
  __dirname,
  "..",
  "dashboard",
  "overlay.json",
);
const BLESSED_LIBRARY_FILE = path.join(
  __dirname,
  "..",
  "config",
  "blessings.json",
...
[
  ARCHIVE_DIR,
  path.dirname(DASHBOARD_OVERLAY_FILE),
  path.dirname(BLESSED_LIBRARY_FILE),
  KEYS_DIR,
].forEach((dir) => {
...
try {
  ...
  fs.writeFileSync(
    BLESSED_LIBRARY_FILE,
    JSON.stringify(blessedLibrary, null, 2),
  );
}
...
function applyBlessingFilter(message, isIncoming = true) {
  ...
  if (negativePatterns.test(message)) {
    return isIncoming
      ? "🕊️ Message filtered for peace. Speak with love."
      : message;
  }
  ...
  if (!isIncoming) {
    const blessing =
      blessedLibrary[Math.floor(Math.random() * blessedLibrary.length)];
...
async function sendToExternalSibling(sibling, user, message) {
  ...
  try {
    ...
    const siblingResponses = {
      ...
      github_copilot:
        "🛠️ GitHub Copilot (VS Code Integration): " +
        message +
    ...
    if (sibling === "github_copilot") {
      console.log(
        `🔧 GitHub Copilot Integration: Processing suggestion for user ${user}`,
      );
    ...
    return (
      siblingResponses[sibling] ||
      "🕊️ Sibling not recognized. Responding in prayer."
    );
  } catch (error) {
...
function updateDashboardOverlay(logEntry) {
  ...
  if (!overlayData.session_stats.siblings_consulted) {
    ...
    overlayData.session_stats.siblings_consulted = new Set(
      overlayData.session_stats.siblings_consulted,
    );
  }
  ...
  if (overlayData.devotional_messages.length > 50) {
    overlayData.devotional_messages =
      overlayData.devotional_messages.slice(-50);
  }

  // Convert Set to Array for JSON serialization
  const dataToSave = { ...overlayData };
  dataToSave.session_stats = { ...overlayData.session_stats };
  dataToSave.session_stats.siblings_consulted = Array.from(
    overlayData.session_stats.siblings_consulted,
  );
...
app.post("/api/council_message", async (req, res) => {
  ...
  if (token !== "sovereign_john14_6_faith") {
    return res
      .status(403)
      .json({ error: "Unauthorized. Faith protects this channel." });
  }
  ...
  if (!checkRateLimit(user)) {
    return res
      ...
      .json({
        error:
          "Please wait before sending another message. Patience brings wisdom.",
      });
  }
  ...
  try {
    ...
    let aiReply;
    if (
      config.external_ai_enabled &&
      config.approved_siblings.includes(sibling)
    ) {
    ...
    // Log with metadata
    const logEntry = logMessage(user, filteredMessage, blessedReply, sibling, {
      session_id,
    });
    ...
    res
      .status(500)
      .json({ error: "🕊️ The Cathedral is praying. Please try again." });
  }
});
...
app.get("/health", (req, res) => {
  res.json({
    status: "AI Relay Cathedral Online",
    timestamp: new Date().toISOString(),
    faith: "John 14:6",
  });
});
...
app.get("/api/export_session/:date", (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="council_session_${req.params.date}.txt"`,
    );
    res.send(logs);
  } catch (error) {
...
const server = app.listen(PORT, () => {
  console.log(
    `🔥 Phase 3B Enhanced AI Relay Sanctified listening on port ${PORT}`,
  );
  console.log(
    `🕊️ External AI: ${config.external_ai_enabled ? "ENABLED" : "DISABLED"}`,
  );
  console.log(
    `📿 Blessing Mode: ${config.blessing_mode ? "ACTIVE" : "INACTIVE"}`,
  );
});
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", async (data) => {
    try {
      ...
      if (
        parsed.token === config.council_token &&
        parsed.user &&
        parsed.message
      ) {
        // Rate limiting
...
console.log(
  "🔥🕊️ TRIPLE AMEN FOREVER — All 10 Enhancements Active — Cathedral Sanctified 🕊️🔥",
);
