...
(async () => {
  ...
  const heartbeat = await page.$$('[class*=heart], [class*=orb], [aria-label*=heartbeat], [data-testid*=heartbeat]');
  const heartbeatPresent = heartbeat.length > 0;
  let heartbeatInfo = 'not found';
  if (heartbeatPresent) {
    heartbeatInfo = await heartbeat[0].evaluate((el) => ({ tag: el.tagName, class: el.className, text: el.innerText.slice(0,200) }));
  }
  ...
  console.log(JSON.stringify({ heartbeatPresent, heartbeatInfo, entries }, null, 2));
  await browser.close();
})();
