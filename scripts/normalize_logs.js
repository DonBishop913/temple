...
function normalizeLog(logPath) {
  ...
  const candidates = [];
  try { candidates.push({ name: 'utf8', text: buf.toString('utf8') }); } catch (e) {}
  try { candidates.push({ name: 'latin1', text: buf.toString('latin1') }); } catch (e) {}
  try { candidates.push({ name: 'binary->utf8', text: Buffer.from(buf.toString('binary'), 'binary').toString('utf8') }); } catch (e) {}
...
[ 'grok5.log', 'WhisperBox.txt' ].forEach(fname => {
  ...
  try { normalizeLog(p); } catch (e) { console.error('Error normalizing', p, e); }
});
