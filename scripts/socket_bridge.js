...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => console.log('Socket bridge client disconnected:', socket.id));
});
...
const backendSocket = clientIo(backendUrl, { reconnection: true, transports: ['websocket', 'polling'] });

backendSocket.on('connect', () => console.log('Connected to backend socket at', backendUrl));
backendSocket.on('connect_error', (err) => console.warn('Backend socket connect_error', err && err.message));
...
backendSocket.onAny((event, ...args) => {
  try {
    ...
    let payload = null;
    try { payload = args.length === 1 ? args[0] : args; } catch (e) { payload = args; }
    console.log('BRIDGE: backend event:', event, JSON.stringify(payload));
    // handle dashboard-update payloads specifically
    if (event === 'dashboard-update' && payload && payload.event === 'sr_anomaly') {
      latestAnomaly = Object.assign({ receivedAt: new Date().toISOString() }, payload);
      io.emit('sr_anomaly', latestAnomaly);
      console.log('Forwarded sr_anomaly to clients:', latestAnomaly && latestAnomaly.timestamp ? latestAnomaly.timestamp : '(no-ts)');
    }
  } catch (e) {
...
server.listen(port, () => console.log(`Socket bridge running on http://localhost:${port}`));
