...
function ngramTokens(s) {
  ...
  const ngrams = [];
  tokens.forEach((t) => {
    for (let i = 0; i < t.length - 1; i++) ngrams.push(t.slice(i, i + 2));
    for (let i = 0; i < t.length - 2; i++) ngrams.push(t.slice(i, i + 3));
  });
  return ngrams;
}
...
let fuzzy = false;
let best = { kw: null, ns: 0, ts: 0, rt: null };
for (const kw of principleKeywords) {
  const ns = ngramSimilarity(normalized, kw);
  if (ns > best.ns) {
    best = { kw, ns, ts: 0, rt: null };
  }
  const kwTokens = kw.split(/\s+/).filter(Boolean);
  for (const kt of kwTokens) {
    for (const respToken of normalized.split(/\s+/).filter(Boolean)) {
      const nst = ngramSimilarity(respToken, kt);
      const tst = tokenSimilarity(respToken, kt);
      if (nst > best.ns) {
        best = { kw, ns: nst, ts: tst, rt: respToken };
      }
      if (nst > 0.24 || tst >= 0.6) {
        fuzzy = true;
        break;
...
console.log(
  "filter result:",
  (hasPrinciple || fuzzy) && !hasNegative ? "ALLOWED" : "FILTERED",
);
