...
try {
        ...
        const ngramTokens = (s) => {
          ...
          tokens.forEach((t) => {
            for (let i = 0; i < t.length - 1; i++)
              ngrams.push(t.slice(i, i + 2));
            for (let i = 0; i < t.length - 2; i++)
              ngrams.push(t.slice(i, i + 3));
          });
          return ngrams;
        };
  ...
  assert(
    yeshuaFilter(n).startsWith("FILTERED"),
    "Negative signals should be filtered",
  );

  // Empty input
  assert(
    yeshuaFilter(null).startsWith("FILTERED"),
    "Null input should be filtered",
  );
  assert(
    yeshuaFilter("").startsWith("FILTERED"),
