...
const publisherArgs = ['c:\\Temple\\council-dashboard\\tools\\stressPublisher.js', '--batch', '8', '--interval', '40', '--count', '2'];

console.log('Starting e2e smoke test');

const pub = spawn(process.execPath, publisherArgs, { stdio: ['ignore', 'pipe', 'pipe'] });
pub.stdout.on('data', d => process.stdout.write('[publisher] ' + d.toString()));
pub.stderr.on('data', d => process.stderr.write('[publisher-err] ' + d.toString()));
...
ws.on('message', (m) => {
  try {
    ...
    if ((msg.type === 'replay' || msg.type === 'replay_pulses') && Array.isArray(msg.payload)) {
    ...
    if (msg.type === 'oversoul_pulse_highlight') {
      console.log('Received highlight echo ids=', msg.ids ? msg.ids.length : 'NA');
      gotHighlightEcho = true;
    }
  } catch (e) { console.error('WS parse err', e && e.message); }
});
...
(async function run() {
  ...
  ws.send(JSON.stringify({ type: 'request_scrub', timestamp: Date.now() - 60000 }));
  ...
  const ids = replayPulses.slice(0, Math.min(5, replayPulses.length)).map(p => p.id);
