...
function publishBatch(batchSize) {
  ...
  for (let i = 0; i < batchSize; i++)
    batch[i] = makePulse(i + batches * batchSize);
  ...
  redis
    .publish(argv.channel, payload)
    .catch((err) => console.error("Publish error", err));
}

console.log(
  `Stress test publisher starting -> ${argv.batch} pulses every ${argv.interval}ms to channel ${argv.channel}`,
);
