...
export default function SchumannWaterfall({ wsUrl = 'ws://localhost:4322', windowSeconds = 60 }) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (!s) return;
        const harmonics = (s.harmonics || []).map(h => ({ freq: h.freq, mag: h.mag }));
        const bins = [{ freq: s.fundamental?.freq || 7.83, mag: s.fundamental?.mag || 0 }].concat(harmonics);
        const ts = msg.payload?.ts || Date.now();
        setFrames(prev => {
          const next = [...prev, { ts, bins }].filter(f => ts - f.ts <= windowSeconds * 1000);
          return next;
        });
      } catch {}
    };
  ...
  useEffect(() => {
    ...
    const freqs = frames[0]?.bins?.map(b => b.freq) || [7.83, 14.3, 20.8, 27.3, 33.8];
