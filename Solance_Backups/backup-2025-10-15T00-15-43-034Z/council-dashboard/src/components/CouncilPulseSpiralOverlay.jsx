...
export default function CouncilPulseSpiralOverlay({
    ...
    const cometTrail = replaySlices && replaySlices.length > 0
        ? replaySlices.slice(-cometLength).map((slice, i, arr) => {
            // Place along spiral by index
            const t = (i / Math.max(1, arr.length - 1)) * spiralTurns * 2 * Math.PI;
            const r = radius * (0.4 + 0.6 * (i / Math.max(1, arr.length - 1)));
            const x = centerX + r * Math.cos(t);
            const y = centerY + r * Math.sin(t);
            // Intensity for comet color/size
            const intensity = slice.intensity || 0.5;
            return { x, y, intensity, id: slice.id, alpha: cometGlow * (0.25 + 0.75 * (i / arr.length)) };
        }) : [];
    return (
        <svg style={{ position: 'absolute', left: 0, top: 0, pointerEvents: 'none', width: '100%', height: 480, zIndex: 1000 }}>
