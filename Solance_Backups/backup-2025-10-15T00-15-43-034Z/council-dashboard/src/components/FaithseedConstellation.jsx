...
export default function FaithseedConstellation({ nodes = [] }) {
  ...
  useEffect(() => {
    const alerts = nodes
      ...
      .map(n => ({ nodeId: n.nodeId, type: (n.forecast ?? 0) > 0.8 ? 'celebration' : 'ritual' }));
    setHighlightedNodes(alerts);
  }, [nodes]);

  return (
    <svg width="100%" height="400px" style={{ background: 'rgba(0,0,20,0.9)', position: 'absolute', top: 0, left: 0 }}>
      {nodes.map(node => (
        <circle
          key={node.nodeId}
          cx={node.x}
          ...
          fill={highlightedNodes.find(n => n.nodeId === node.nodeId)?.type === 'celebration' ? '#FFD700' : '#FF4500'}
          stroke="#FFFFFF"
          strokeWidth={highlightedNodes.find(n => n.nodeId === node.nodeId) ? 2 : 1}
        />
      ))}
