 ...
 */
export default function QuantumLeapReplay({ batchSize = 500, intervalMs = 100, autoStart = true }) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      try { ws.send(JSON.stringify({ type: 'request_replay', percent: 1, batchSize })); } catch (e) {}
    };
    ws.onmessage = (ev) => {
      try {
    ...
    ws.onerror = () => { /* ignore */ };
    const t = setTimeout(() => { try { ws.close(); } catch (e) {} }, 5000);
    return () => { clearTimeout(t); try { ws.close(); } catch (e) {} };
  }, [batchSize]);
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (!p) return;
      try { highlight && highlight([p.id || p.unique || (p.t || p.timestamp)]); } catch (e) {}
      try { scrub && scrub(p.timestamp || p.t || Date.now()); } catch (e) {}
      idxRef.current += 1;
    }, intervalMs);
