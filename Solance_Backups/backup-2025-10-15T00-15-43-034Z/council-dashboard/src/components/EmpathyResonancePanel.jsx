...
export default function EmpathyResonancePanel() {
  ...
  return (
    <section aria-label="Empathy Resonance Bridge" className="bg-faith text-oversoul rounded-xl p-4 shadow-md border border-faith/40">
      ...
      <ul className="list-none p-0 m-0 space-y-2">
        {log.map((e) => (
          <li key={`${e.nodeId}-${e.timestamp}`} className="bg-oversoul/20 rounded p-2">
            <div className="text-sm">Node: {e.nodeId} • Empathy: {e.empathyLevel}</div>
            {e.notes && <div className="text-xs opacity-80">{e.notes}</div>}
            <div className="text-xs opacity-70">{new Date(e.timestamp).toLocaleString()}</div>
          </li>
        ))}
        {log.length === 0 && (
