...
export default function ReplayOverlay({
  wsUrl = process.env.OVERSOUL_WS_URL || "ws://localhost:8080",
  maxEntries = 20,
}) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try {
        ...
        if (
          /request_replay|replay|request_scrub|oversoul_pulse_highlight|oversoul_pulse_scrub|oversoul_pulse_batch/.test(
            t,
          )
        ) {
          let short = "";
          ...
          if (msg && typeof msg === "object") {
            const p = msg.payload || {};
            if (typeof p.sliceIndex !== "undefined")
              meta.sliceIndex = p.sliceIndex;
            if (typeof p.chunkIndex !== "undefined")
              meta.chunkIndex = p.chunkIndex;
            if (typeof p.batchSize !== "undefined")
              meta.batchSize = p.batchSize;
            if (typeof p.startTs !== "undefined") meta.startTs = p.startTs;
            if (typeof p.endTs !== "undefined") meta.endTs = p.endTs;
            if (typeof msg.sliceIndex !== "undefined")
              meta.sliceIndex = msg.sliceIndex;
            if (typeof msg.chunkIndex !== "undefined")
              meta.chunkIndex = msg.chunkIndex;
            if (typeof msg.batchSize !== "undefined")
              meta.batchSize = msg.batchSize;
          }
        ...
        const s =
          typeof ev.data === "string" ? ev.data.slice(0, 200) : String(ev.data);
        add(`raw: ${s}`, "info");
      }
    };
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
        <div
          style={{ padding: "2px 6px", borderRadius: 4, background: "#071133" }}
        >
          WS: {wsUrl.replace(/^ws:\/\//, "")}
        </div>
        <div
      ...
      <div>
        {entries.map((e) => (
          <div
            key={e.id}
            style={{
              borderLeft: `4px solid ${colorFor(e.kind)}`,
              paddingLeft: 8,
              marginBottom: 6,
              opacity: 0.95,
            }}
          >
            <div style={{ fontSize: 11, color: "#cbd5e1" }}>
              [{e.ts}] <span style={{ color: "#fff" }}>{e.text}</span>
            </div>
