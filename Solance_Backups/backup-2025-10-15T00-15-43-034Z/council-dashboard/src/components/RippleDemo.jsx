...
export function QuantumRippleField(container) {
  ...
  this.spawnRipple = (pulse) => {
    ...
    r.id =
      pulse.id ||
      `pulse_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  ...
  this.highlight = (ids) => {
    /* not implemented: could pulse positions for known ids */
  };
  this.scrub = (ts) => {
    /* optional scrub visual */
  };
...
export default function RippleDemo({ wsUrl = "ws://localhost:8080" }) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      setConnected(true);
      console.log("[RippleDemo] ws open");
    };
    ws.onclose = () => {
      setConnected(false);
      console.log("[RippleDemo] ws closed");
    };
      ...
      try {
        ...
        if (msg.type === "oversoul_pulse_batch" && Array.isArray(msg.payload)) {
          ...
          const payload = Array.isArray(msg.payload)
            ? msg.payload
            : msg.payload && msg.payload.pulses
              ? msg.payload.pulses
              : [];
          payload.forEach((p) =>
            field.spawnRipple({ id: p.id, color: p.color, x: p.x, y: p.y }),
          );
        }
      } catch (e) {
        console.error("[RippleDemo] parse error", e);
      }
    };
    wsRef.current = ws;
    ...
    const id = setInterval(() => {
      ...
      try {
        setStats(
          field.renderer || { fps: 0, pulsesReceived: 0, pulsesThisSecond: 0 },
        );
      } catch (e) {}
    }, 300);
    ...
    >
      ...
      >
        ...
        <button
          onClick={() => {
            if (fieldRef.current) {
              for (let i = 0; i < 10; i++) fieldRef.current.spawnRipple({});
            }
          }}
        >
          Burst
        ...
        <button onClick={() => sendRequestScrub(10000)}>
          Request Scrub -10s
        </button>
        <div style={{ marginLeft: "auto" }}>
          FPS: {stats.fps}, Pulses/sec: {stats.pulsesThisSecond}
        </div>
      </div>
    </div>
  );
