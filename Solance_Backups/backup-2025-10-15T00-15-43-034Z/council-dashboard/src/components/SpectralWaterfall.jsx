...
const SpectralWaterfall = ({ wsUrl, fftFrames = [], schumannHz, overlay432Hz = false }) => {
  ...
  return (
    <div aria-label="Schumann resonance spectral waterfall chart" role="region" tabIndex={0} style={{ outline: 'none' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <h2 style={{ margin: 0, fontSize: '1rem' }}>Schumann Resonance FFT Waterfall</h2>
        <button
          type="button"
          aria-label="Toggle high contrast colors"
          aria-pressed={colorscale !== 'Viridis'}
          onClick={() => setColorscale((c) => (c === 'Viridis' ? 'Greys' : 'Viridis'))}
      ...
      <Plot
        ...
        layout={{
          ...
          yaxis: { title: 'Frequency (Hz)', showgrid: true, autorange: 'reversed' },
          autosize: true,
        }}
          ...
          <div>
            ...
            <ul>
              {latestFrame.frequencies.map((f, i) => (
                <li key={`${f}-${i}`}>{f} Hz: {Number(latestFrame.magnitudes[i]).toFixed(2)}</li>
              ))}
            </ul>
          </div>
        ) : (
