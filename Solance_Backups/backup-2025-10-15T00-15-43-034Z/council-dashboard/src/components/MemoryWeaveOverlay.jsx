...
export default function MemoryWeaveOverlay({ enabled = true, nodeId = "N1" }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/venice/suggestions/${nodeId}`)
      .then((r) => r.json())
      ...
      >
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
            {archive.map((entry, i) => (
              <li key={i} style={{ marginBottom: 2 }}>
                <span style={{ color: "#0cf" }}>{entry.timestamp}:</span>{" "}
                <span>{entry.action}</span>{" "}
                <span style={{ color: "#ccc" }}>
                  {entry.sentiment != null ? `(${entry.sentiment})` : ""}
                </span>
              </li>
            ))}
          </ul>
            ...
            <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
              {live.map((entry, i) => (
                <li key={`live-${i}`} style={{ marginBottom: 2 }}>
                  ...
                  <span>
                    {Array.isArray(entry.glyphs) ? entry.glyphs.join("") : ""}
                  </span>
                </li>
              ))}
            </ul>
        ...
        <div style={{ fontSize: 13 }}>
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
            {patterns.map((p, i) => (
              <li key={i} style={{ marginBottom: 2 }}>
                <span style={{ color: "#ff0" }}>{p.type}</span>: {p.count}{" "}
                <span style={{ color: "#0f0" }}>impact: {p.impactScore}</span>
              </li>
            ))}
          </ul>
        </div>
