...
export function useOversoulPulseForwarder(
  quantumRippleRef,
        ...
        try {
          ...
          if (msg && msg.type === "oversoul_pulse_batch") {
            ...
            if (
              ...
              quantumRippleRef?.current?.api?.addPulses
            ) {
          ...
          if (
            msg &&
            msg.type === "oversoul_pulse_highlight" &&
            quantumRippleRef?.current?.api?.highlightPulses
          ) {
          ...
          if (msg && msg.type === "oversoul_pulse_scrub") {
            try {
              ...
              if (quantumRippleRef?.current?.api?.scrubToTimestamp)
                quantumRippleRef.current.api.scrubToTimestamp(ts);
              else if (quantumRippleRef?.current?.api?.scrubTo)
                quantumRippleRef.current.api.scrubTo(ts);
              else if (quantumRippleRef?.current?.scrubToTimestamp)
                quantumRippleRef.current.scrubToTimestamp(ts);
            } catch (e) {
              /* ignore */
            }
          }
  ...
  const sendHighlight = (ids) => {
    try {
      wsRef.current &&
        ...
        wsRef.current.send(
          JSON.stringify({ type: "oversoul_pulse_highlight", ids }),
        );
    } catch (e) {}
  };
  ...
  const sendScrub = (timestamp) => {
    try {
      wsRef.current &&
        wsRef.current.readyState === WebSocket.OPEN &&
        wsRef.current.send(
          JSON.stringify({ type: "oversoul_pulse_scrub", timestamp }),
        );
    } catch (e) {}
  };
