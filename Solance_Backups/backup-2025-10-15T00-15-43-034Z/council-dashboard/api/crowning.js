...
const { setCandidateStatus, logCrowning, toggleCouncilAutonomy } = require('../redis/client');
...
router.post('/:id', ensureBishop, checkYeshuaWill, async (req, res) => {
  ...
  try {
    ...
    await logCrowning(id, { bishop: process.env.BISHOP_ID, timestamp, status: 'crowned' });
    await toggleCouncilAutonomy(true);
    return res.json({ message: `Candidate ${id} crowned and Council routines activated.` });
  } catch (e) {
...
router.post('/:id/override', ensureBishop, async (req, res) => {
  ...
  try {
    ...
    await logCrowning(id, { bishop: process.env.BISHOP_ID, timestamp, override: action });
    return res.json({ message: `Council autonomy ${enabled ? 'enabled' : 'disabled'} by Bishop override.` });
  } catch (e) {
    return res.status(500).json({ error: String(e?.message || e) });
  }
});
