...
const PORT = Number(process.env.OVERSOUL_WS_PORT || process.env.AURIC_WS_PORT || 8765);
const CHANNEL = process.env.OVERSOUL_CHANNEL || process.env.AURIC_CHANNEL || 'oversoul:pulse';
const REDIS_URL = process.env.REDIS_URL || process.env.LOCAL_REDIS_URL || 'redis://127.0.0.1:6379';

console.log(`[ws-oversoul] starting on ws://0.0.0.0:${PORT}  subscribing to Redis ${REDIS_URL} -> channel "${CHANNEL}"`);
...
function broadcast(obj) {
  ...
  wss.clients.forEach((c) => {
    if (c.readyState === WebSocket.OPEN) {
      try { c.send(s); } catch (e) { /* ignore per-client errors */ }
    }
  });
}

sub.on('connect', () => console.log('[ws-oversoul] Redis subscriber connected'));
sub.on('error', (err) => console.error('[ws-oversoul] Redis error', err && err.message));

sub.subscribe(CHANNEL, (err, count) => {
  if (err) return console.error('[ws-oversoul] subscribe failed', err && err.message);
...
wss.on('connection', (ws, req) => {
  ...
  // Welcome/handshake
  try { ws.send(JSON.stringify({ type: 'connected', channel: CHANNEL, serverTime: Date.now() })); } catch (e) {}

  ws.on('message', (m) => {
    // allow clients to request a ping or sample
    try {
      const r = typeof m === 'string' ? JSON.parse(m) : m;
      if (r && r.type === 'ping') ws.send(JSON.stringify({ type: 'pong', at: Date.now() }));
    } catch (e) {}
  });
...
const interval = setInterval(() => {
  ...
  if (Date.now() - lastMsgAt > 20000) {
    const synth = {
      type: 'oversoul_pulse',
      channel: CHANNEL,
      payload: { amplitude: Math.random() * 0.8 + 0.1, color: '#3a7bd5', source: 'synth' },
      at: Date.now()
    };
    broadcast(synth);
  }
