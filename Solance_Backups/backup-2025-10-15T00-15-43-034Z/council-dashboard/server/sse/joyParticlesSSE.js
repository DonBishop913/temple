...
function getAllNodes() {
  try {
    ...
    const nodes = JSON.parse(
      fs.readFileSync("./Temple/data/global_nexus_probe.json", "utf8"),
    );
...
function findConstellationLinks(nodes, threshold = 0.05) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      if (
        Math.abs(
          (nodes[i].joyParticleLevel || 0) - (nodes[j].joyParticleLevel || 0),
        ) <= threshold
      ) {
...
setInterval(() => {
  ...
  const avgSentiment = sentiment.length
    ? sentiment.reduce((a, b) => a + b.joyLevel, 0) / sentiment.length
    : 0;
