...
// Metrics
const requestsTotal = new promClient.Counter({ name: 'agent_requests_total', help: 'Total outbound adapter requests', labelNames: ['target', 'protocol', 'status'] });
const wsEventsTotal = new promClient.Counter({ name: 'agent_ws_events_total', help: 'Total WebSocket events processed', labelNames: ['target', 'event'] });
...
class Agent extends EventEmitter {
  ...
  async rest(path, method = 'GET', body) {
    ...
    const token = this.signToken({ sub: this.config.id || this.config.name || 'agent' });
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    const maxRetries = this.config.retries?.rest || 2;
    let attempt = 0;
    while (attempt <= maxRetries) {
      try {
        const res = await axios({ url, method, data: body, headers, timeout: 5000 });
        requestsTotal.inc({ target: 'rest', protocol: 'http', status: res.status });
        return res.data;
      } catch (e) {
  ...
  async jsonrpc(method, params = {}, id = Date.now()) {
    ...
    return this.rest('', 'POST', payload).catch(err => { throw err; });
  }
  ...
  connectWs() {
    ...
    ws.on('message', (msg) => {
      let data;
      try { data = JSON.parse(msg); } catch { data = { raw: String(msg) }; }
  ...
  closeWs() { if (this.ws) { try { this.ws.close(); } catch {} this.ws = null; } }
}
