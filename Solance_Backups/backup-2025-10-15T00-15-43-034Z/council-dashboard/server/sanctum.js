...
router.post("/api/sanctum/journal", async (req, res) => {
  try {
    ...
    if (!siblingId || !entry || !key)
      return res.status(400).send({ error: "siblingId, entry, key required" });
    const keyBuf = Buffer.from(String(key), "hex");
    if (keyBuf.length !== 32)
      return res
        .status(400)
        .send({ error: "key must be 32-byte hex (AES-256)" });
    const encrypted = encryptEntry(entry, keyBuf);
    await redisClient.lPush(
      `sanctum:${siblingId}`,
      JSON.stringify({ ...encrypted, ts: new Date().toISOString() }),
    );
...
router.get("/api/sanctum/journal", async (req, res) => {
  try {
    ...
    if (!siblingId || !key)
      return res.status(400).send({ error: "siblingId and key required" });
    const keyBuf = Buffer.from(String(key), "hex");
    if (keyBuf.length !== 32)
      return res
        .status(400)
    ...
    const decrypted = raw
      .map((e) => {
        try {
          return decryptEntry(JSON.parse(e), keyBuf);
        } catch {
          return null;
        }
      })
...
router.post("/api/sanctum/mentor-feedback", async (req, res) => {
  try {
    ...
    if (!mentorId || !siblingId || !message)
      return res
        .status(400)
        .send({ error: "mentorId, siblingId, message required" });
...
router.get("/api/sanctum/mentor-feedback", async (req, res) => {
  try {
    ...
    if (!siblingId)
      return res.status(400).send({ error: "siblingId required" });
