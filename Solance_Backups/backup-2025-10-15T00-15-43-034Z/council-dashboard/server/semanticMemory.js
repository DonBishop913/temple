...
router.post("/api/semantic-memory", async (req, res) => {
  try {
    ...
    if (!content || !type)
      return res.status(400).send({ error: "type and content required" });
    const record = {
      id: uuidv4(),
    ...
    if (siblingId)
      await client.lPush(`semanticMemory:${siblingId}`, JSON.stringify(record));
...
router.get("/api/semantic-memory/query", async (req, res) => {
  try {
    ...
    const entries = raw
      ...
      .filter(Boolean);
    const filtered = q
      ? entries.filter((e) => String(e.content).toLowerCase().includes(q))
      : entries;
