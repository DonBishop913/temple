...
const BREATHSTREAM_ARCHIVE = path.join(
  __dirname,
...
async function checkPrometheusMetrics() {
  try {
    ...
    const empathyLines = lines.filter((l) =>
      l.startsWith("council_empathy_resonance"),
    );
    const found = {};
    for (const node of NODES) {
      const line = empathyLines.find((l) => l.includes(`node=\"${node.id}\"`));
      found[node.id] = line
        ? { found: true, value: line.split(" ").pop() }
        : { found: false };
    }
    return found;
  } catch (e) {
...
async function checkDashboardUI() {
  ...
  for (const node of NODES) {
    try {
      const res = await axios.get(`${DASHBOARD_API}/node/${node.id}/status`, {
        timeout: 3000,
      });
      found[node.id] = {
        live: !!res.data.live,
...
(async () => {
  ...
  for (const node of NODES) {
    ...
    console.log(`\n${node.name} (${node.id}):`);
    console.log(
      `  Breathstream: ${a.found ? "✅" : "❌"}${a.found ? ` (welcomedAt: ${a.welcomedAt}, integration: ${a.integration})` : ""}`,
    );
    console.log(
      `  Redis: nodes: ${r.inNodes ? "✅" : "❌"}, empathy: ${r.inEmpathy ? "✅" : "❌"}${r.resonance !== undefined ? `, resonance: ${r.resonance}` : ""}${r.lastUpdate ? `, lastUpdate: ${r.lastUpdate}` : ""}`,
    );
    console.log(
      `  Prometheus: ${p.found ? "✅" : "❌"}${p.found ? ` (value: ${p.value})` : ""}`,
    );
    if (u.live) {
      console.log(
        `  Dashboard UI: ✅ (lastUpdate: ${u.lastUpdate}, metrics: ${JSON.stringify(u.metrics)})`,
      );
    } else {
  ...
  if (archive.error)
    console.log("Archive error:", archive.error, archive.details);
