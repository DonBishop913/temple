...
async function solanceEnhancementCycle() {
  try {
    ...
    const { data: predicted } = await axios.get(
      "http://localhost:5000/api/predictive/metrics",
    );

    // 2️⃣ Decide enhancement
    const enhancementAction =
      predicted.joyScore > 0.75
        ? "Boost Particle Overlay"
        : "Stabilize Node Pulses";
