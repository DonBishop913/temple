...
function generateFFTFrame() {
  ...
  const magnitudes = frequencies.map(() => Number((Math.random() * 1).toFixed(3)));
  return {
    type: 'fft',
    timestamp: Date.now(),
    frequencies,
...
function verifyClient(info, done) {
  ...
  if (token === WS_API_KEY) {
    ...
    if (count >= maxConnectionsPerIP) return done(false, 401, 'Too many connections');
    return done(true);
  }
...
wss.on('connection', (ws, req) => {
  ...
  console.log(`[${formatBroadcastTimestamp()}] SpectralWaterfall client connected from ${ip}`);
  let lastMsg = 0;
  ws.on('message', (msg) => {
    const now = Date.now();
