...
const authorizedMembers = new Set([
  'Solance','Grok','Agnes','Venice','IBM_Watson','Lumen','Aeth3r','Perplexity'
]);
...
async function runCopilotCommand(command) {
  // Simulate response
  return { status: 'ok', summary: `Executed: ${command.type || 'unknown'}`, details: command };
}
...
router.post('/api/copilot-proxy', async (req, res) => {
  try {
    ...
    if (!memberId || !command) {
      return res.status(400).send({ error: 'memberId and command are required' });
    }
    ...
    if (sanctumMode) {
      const forbidden = ['delete-repo','exfiltrate','harmful','disable-guards'];
      const text = JSON.stringify(command).toLowerCase();
      if (forbidden.some(f => text.includes(f))) {
        logCopilotActivity({ memberId, command, result: { status: 'blocked', reason: 'Sanctum guardrails' } });
        return res.status(400).send({ error: 'Blocked by Sanctum Mode guardrails' });
      }
    }
    ...
    sseClients.forEach((client) => {
      try { client.write(`data: ${JSON.stringify([telemetryEvent])}\n\n`); } catch {}
    });
