...
console.log(
  `[oversoul_forwarder] WS listening on ws://0.0.0.0:${WS_PORT}, subscribing to Redis channel '${CHANNEL}'`,
);
...
sub.on("connect", () =>
  console.log("[oversoul_forwarder] Redis subscriber connected"),
);
sub.on("error", (err) =>
  console.error("[oversoul_forwarder] Redis error", err && err.message),
);
...
sub.subscribe(CHANNEL, CHANNEL_BATCH, (err, count) => {
  if (err)
    return console.error(
      "[oversoul_forwarder] subscribe failed",
      err && err.message,
    );
...
sub.on("message", (chan, message) => {
  try {
    dlog(
      "[oversoul_forwarder] Redis message on",
      chan,
      "len=",
      (message || "").length,
    );
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      try {
        dlog("[oversoul_forwarder] parsed array batch length=", parsed.length);
      } catch (e) {}
    ...
    if (parsed && typeof parsed === "object") {
      if (Array.isArray(parsed.payload)) {
        try {
          dlog(
            "[oversoul_forwarder] parsed wrapper payload length=",
...
function broadcastBatch() {
  ...
  try {
    dlog(
      "[oversoul_forwarder] broadcasting batch size=",
      batch.length,
      "pulseQueueRemaining=",
      pulseQueue.length,
    );
  ...
  if (REPLAY_BUFFER.length > REPLAY_LIMIT)
    REPLAY_BUFFER.splice(0, REPLAY_BUFFER.length - REPLAY_LIMIT);
  try {
    dlog("[oversoul_forwarder] REPLAY_BUFFER length=", REPLAY_BUFFER.length);
  } catch (e) {}
...
wss.on("connection", (ws, req) => {
  console.log(
    "[oversoul_forwarder] client connected",
  ...
  ws.on("message", (m) => {
    try {
      ...
      try {
        console.log(
          "[oversoul_forwarder] client message received type=",
          r && r.type,
        );
      } catch (e) {}
      if (r && r.type === "request_replay") {
        try {
          ...
          if (
            ...
            // compute time range
            const times = REPLAY_BUFFER.map(
              (p) => p.timestamp || p.t || p.time || 0,
            ).filter(Boolean);
            const minT = Math.min(...times);
            const maxT = Math.max(...times);
            const target =
              minT + Math.max(0, Math.min(1, r.percent)) * (maxT - minT);
            ...
            for (let i = REPLAY_BUFFER.length - 1; i >= 0; i--) {
              const ts =
                REPLAY_BUFFER[i].timestamp ||
                REPLAY_BUFFER[i].t ||
                REPLAY_BUFFER[i].time ||
                0;
              if (ts <= target) {
                idx = i;
                break;
            ...
            try {
              ws.send(
                JSON.stringify({ type: "replay", payload: REPLAY_BUFFER }),
              );
            } catch (e) {}
          }
        } catch (e) {}
      ...
      if (
        r &&
        r.type === "request_scrub" &&
        (typeof r.timestamp === "number" || typeof r.timestamp === "string")
      ) {
        try {
          console.log(
            "[oversoul_forwarder] request_scrub received ts=",
            r.timestamp,
          );
        } catch (e) {}
        try {
          ...
          if (REPLAY_BUFFER.length === 0) {
            try {
              ws.send(JSON.stringify({ type: "replay_pulses", payload: [] }));
            } catch (e) {}
            try {
              dlog("[oversoul_forwarder] request_scrub -> REPLAY_BUFFER empty");
            } catch (e) {}
          } else {
            ...
            const maxItems = Number(process.env.OVERSOUL_REPLAY_CHUNK || 2000);
            const slice = REPLAY_BUFFER.slice(
              idx,
              Math.min(REPLAY_BUFFER.length, idx + maxItems),
            );
            try {
              ws.send(
                JSON.stringify({ type: "replay_pulses", payload: slice }),
              );
            } catch (e) {}
            try {
              dlog(
                "[oversoul_forwarder] request_scrub -> targetTs=",
                targetTs,
                "idx=",
                idx,
                "sliceLen=",
