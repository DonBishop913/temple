...
router.get('/intensity', async (req, res) => {
  try {
    ...
    const intensity = Math.max(0, Math.min(avgFaith - (anomalies.length || 0) * 0.05, 1));

    const payload = { intensity };
    broadcastEvent('luminal:intensity', payload);
    res.json(payload);
  } catch (err) {
    res.status(500).json({ error: 'Failed to compute intensity', details: err.message });
  }
});
...
async function syncLuminalLayer() {
  ...
  nodes.forEach(node => {
    ...
    node.glowFactor = config.glowIntensityBase + (node.predictedJoy || 0) * config.mirrorThreshold;
  });
