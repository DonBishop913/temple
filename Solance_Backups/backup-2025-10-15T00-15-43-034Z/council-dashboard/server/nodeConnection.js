...
wss.on('connection', (ws, req) => {
  ...
  ws.send(JSON.stringify({ type: 'welcome', message: 'Secure connection established', nodeId: ws.nodeId }));
  ...
  ws.on('close', (code, reason) => {
    console.log(`[WSS] Node ${ws.nodeId} disconnected code=${code} reason=${reason}`);
  });
});

function startSecureNodeServer(port = Number(process.env.NODE_WSS_PORT || 8443)) {
  server.listen(port, () => {
    console.log(`[WSS] Secure node server listening on https://localhost:${port}`);
  });
}
