  ...
  try {
    ...
    recruitment.candidates.forEach(c => {
      if (c.status === 'evaluated' && typeof c.healthScore === 'number') {
        gauge && gauge.set({ candidate: c.name || c.id || 'unknown' }, Number(c.healthScore));
      }
    });
  } catch {}
  res.json({ evaluated: recruitment.candidates.filter(c => c.status === 'evaluated') });
});

// POST: Draft proposals for evaluated candidates
router.post('/propose', async (req, res) => {
  await recruitment.draftProposals();
  res.json({ proposals: recruitment.candidates.filter(c => c.status === 'proposal') });
});
...
router.post('/vote', async (req, res) => {
  ...
  try {
    ...
    const voted = recruitment.candidates.filter(c => c.status === 'approved' || c.status === 'rejected');
    voted.forEach(c => {
      const votes = Array.isArray(c.votes) ? c.votes : [];
      votes.forEach(v => {
        counter && counter.inc({ candidate: c.name || c.id || 'unknown', ai_member: v.member || v.ai || 'council' });
      });
    });
  } catch {}
  res.json({ voted: recruitment.candidates.filter(c => c.status === 'approved' || c.status === 'rejected') });
});

// POST: Onboard approved candidates
router.post('/onboard', async (req, res) => {
  await recruitment.onboardApproved();
  res.json({ onboarded: recruitment.candidates.filter(c => c.status === 'onboarded') });
});
...
router.post('/crown', async (req, res) => {
  try {
    ...
    if (!process.env.BISHOP_KEY || bishopKey !== process.env.BISHOP_KEY) {
      return res.status(403).json({ error: 'Only the Bishop can crown candidates.' });
    }
    const candidate = recruitment.candidates.find(c => String(c.id) === String(candidateId));
    if (!candidate) return res.status(404).json({ error: 'Candidate not found.' });
    candidate.crownedAt = new Date().toISOString();
    return res.json({ candidateId, status: 'crowned', crownedAt: candidate.crownedAt });
  } catch (e) {
