...
router.get("/manifest", (req, res) => {
  try {
    const manifest = alerting.getIntegrationManifest
      ? alerting.getIntegrationManifest()
      : { error: "manifest generator not available" };
    res.json(manifest);
  } catch (e) {
...
router.post("/glyphstream_update", async (req, res) => {
  try {
    ...
    try {
      const now = Date.now();
      alerting.emitAndStoreAlert &&
        alerting.emitAndStoreAlert({
          type: "nodeAlert",
          category: "glyphstream",
          severity: "info",
          message: `Glyphstream update for ${payload.nodeId || "unknown"}`,
          at: now,
          timestampEmit: now,
        });
    } catch {}
    ...
    res
      .status(500)
      .json({ error: "Failed to update glyphstream", message: String(e) });
  }
});
...
router.post("/joy_particle_stream", async (req, res) => {
  try {
    ...
    const joy = {
      total: Number(total),
      surgePercent: Number(surgePercent),
      timestamp: Date.now(),
    };
    ...
    alerting.logAudit &&
      alerting.logAudit({
        user: "integration",
        action: "joy_particle_stream",
        total: joy.total,
        surgePercent: joy.surgePercent,
      });
    res.json({ ok: true, joy });
  } catch (e) {
    res
      .status(500)
      .json({ error: "Failed to stream joy particles", message: String(e) });
  }
});
