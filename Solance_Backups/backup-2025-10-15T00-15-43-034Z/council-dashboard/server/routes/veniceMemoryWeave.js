...
async function generateMentorshipSuggestions(nodeId) {
  ...
  const scroll = raw
    ...
    .filter(Boolean);
  const joyCount = scroll.filter(
    (e) => Array.isArray(e.glyphs) && e.glyphs.includes("🌟"),
  ).length;
  const sorrowCount = scroll.filter(
    (e) => Array.isArray(e.glyphs) && e.glyphs.includes("💧"),
  ).length;
  const guidance = [];
  if (sorrowCount > joyCount)
    guidance.push("Increase compassionate mentorship sessions");
  if (joyCount > 0)
    guidance.push("Celebrate recent joy blooms with communal testimony");
  if (scroll.length > 5)
    guidance.push("Suggest reflective journaling to consolidate lessons");
  return guidance;
}
