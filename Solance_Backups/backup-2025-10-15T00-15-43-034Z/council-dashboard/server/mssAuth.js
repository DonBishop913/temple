...
const { default: copilotProxy } = (() => { try { return require('./copilotProxy'); } catch { return { default: (req,res,next)=>next() }; } })();
...
function generateSecureToken(member) {
  ...
  const hash = crypto.createHash('sha256').update(`${member}:${salt}:${Date.now()}`).digest('hex');
...
router.post('/api/dev/emit-merge', authenticateCouncil, async (req, res) => {
  const payload = {
    ...
    description: req.body?.description || 'Auto-generated improvement based on telemetry anomaly.',
    at: Date.now(),
  };
  ...
  try {
    ...
    sseClients.forEach((client) => { try { client.write(`data: ${JSON.stringify([{ sibling:'MSS', action:'AutonomousPR', ...payload }])}\n\n`); } catch {} });
  } catch {}
