...
async function getCandidateStatus(id) {
  try {
    return await redis.get(`candidate:${id}.status`);
  } catch {
    return null;
  }
}
async function setCandidateStatus(id, status) {
  try {
    await redis.set(`candidate:${id}.status`, status);
  } catch {}
}
async function logCrowning(id, logEntry) {
  try {
    await redis.rpush(`crownLog:${id}`, JSON.stringify(logEntry));
  } catch {}
}
async function toggleCouncilAutonomy(enabled) {
  try {
    await redis.set("council:autonomy", enabled ? "enabled" : "disabled");
  } catch {}
}
