...
async function main() {
  ...
  const summary = {
    at: now,
    count: nodes.length,
    avgGlow: nodes.length
      ? nodes.reduce((s, n) => s + Number(n.glowFactor || 0), 0) / nodes.length
      : 0,
  };
  await redis.set("lumen:glow:snapshot", JSON.stringify({ at: now, nodes }));
  fs.appendFileSync(
    auditPath,
    `[${now}] Lumen glow snapshot: ${summary.count} nodes, avg ${summary.avgGlow.toFixed(3)}\n`,
  );
  console.log(
    `Committed Lumen glow snapshot. Nodes=${summary.count}, avg=${summary.avgGlow.toFixed(3)}`,
  );
}
