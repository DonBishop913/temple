...
async function registerNodeWithStarlink(node) {
  ...
  const res = await axios.post('https://api.starlink.com/nexus/register', payload, {
    headers: {
  ...
  for (const node of nodes) {
    try {
      ...
      if (result && result.success) {
        ...
        healthEmitter && healthEmitter.emit('nexusConnected', { nodeId: node.id });
        results.push({ node, success: true });
      } else {
        logAudit({ user: 'agnes', action: 'nexus_failed', node: node.id, error: result?.error });
        results.push({ node, success: false, error: result?.error });
      }
    } catch (e) {
      logAudit({ user: 'agnes', action: 'nexus_error', node: node.id, error: String(e && e.message || e) });
      results.push({ node, success: false, error: String(e && e.message || e) });
    }
  }
  return results;
}
