...
function connectOnce() {
  return new Promise((resolve, reject) => {
    ...
    const watchdog = setTimeout(
      () => fail(new Error("No data within 3s")),
      3000,
    );
    ...
    ws.on("message", (msgRaw) => {
      try {
        ...
        if (!msg || typeof msg !== "object")
          throw new Error("non-object message");
        if (!("type" in msg) || !("payload" in msg))
          throw new Error("missing type/payload");
        types.add(msg.type);
        // targeted payload validations for two representative streams
        if (msg.type === "planetary" && !Array.isArray(msg.payload))
          throw new Error("planetary payload invalid");
        if (msg.type === "nodes" && !Array.isArray(msg.payload))
          throw new Error("nodes payload invalid");
        if (types.size >= 3) {
          // first burst usually includes multiple types quickly
