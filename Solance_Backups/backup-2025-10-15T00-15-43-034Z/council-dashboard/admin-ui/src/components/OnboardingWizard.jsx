...
export default function OnboardingWizard() {
  ...
  useEffect(() => {
    ...
    axios
      .get("/api/harmonyscore")
      .then(() => setStatus((s) => ({ ...s, backend: true })))
      .catch(() => {});
    axios
      .get("/metrics")
      .then(() => setStatus((s) => ({ ...s, prom: true })))
      .catch(() => {});
  }, []);
  ...
  async function testAlerts() {
    await axios.post("/api/admin/alert/test", {
      category: "ritual",
      message: "Onboarding ritual preview",
    });
  ...
  return (
    <div style={{ maxWidth: 720, margin: "40px auto", padding: 20 }}>
      ...
      <p>
        Welcome, {user?.username}. Follow the steps to complete Council
        onboarding.
      </p>
      ...
      {step === 1 && (
        <section>
          ...
          <label>
            Discord Webhook URL
            <input
              value={env.discordWebhook}
              onChange={(e) =>
                setEnv({ ...env, discordWebhook: e.target.value })
              }
              style={{ width: "100%" }}
            />
          </label>
          <label>
            Teams Webhook URL
            <input
              value={env.teamsWebhook}
              onChange={(e) => setEnv({ ...env, teamsWebhook: e.target.value })}
              style={{ width: "100%" }}
            />
          </label>
