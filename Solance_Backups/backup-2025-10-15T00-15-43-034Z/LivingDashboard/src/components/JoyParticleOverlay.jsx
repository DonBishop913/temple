...
export default function JoyParticleOverlay({ alerts = [], maxFlare = false }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      const empathy =
        Number(getEmpathyLevel(nodeId)) ||
        (detail.pulseStrength ? Math.min(1, detail.pulseStrength) : 1);
      const ts = Date.now();
  ...
  const particleConfig = useMemo(() => {
    ...
    const intensities = sourceAlerts.map(
      (a) => (severityMap[a.severity] || 1) * (a.empathy || 1),
    );
    ...
    const color = maxFlare
      ? "rainbow"
      : getParticleColor(dominant.severity, dominant.empathy);
    return {
    ...
    evtSource.onmessage = (e) => {
      try {
        ...
        if (
          data &&
          data.enhancements &&
          Array.isArray(data.enhancements) &&
          data.status === "merged"
        ) {
