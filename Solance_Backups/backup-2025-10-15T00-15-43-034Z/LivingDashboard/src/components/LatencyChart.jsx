...
export default function LatencyChart({
  ...
  const xScale = (i) =>
    padding + (i * (width - 2 * padding)) / Math.max(1, nodes.length - 1);
  const yScale = (ms) =>
    height - padding - (ms * (height - 2 * padding)) / maxLatency;
  const color = (i) => `hsl(${(i * 60) % 360}, 70%, 50%)`;

  const points = nodes.map((nodeId, i) => {
    const latest = [...latencies]
      .reverse()
      .find((l) => (l.nodeId || "global") === nodeId);
    const x = xScale(i);
  ...
  return (
    <svg
      ...
      height={height}
      role="img"
      aria-label="Alert Propagation Latency Chart"
    >
      ...
      {points.map((p, idx) => (
        <g key={idx}>
          ...
          <text
            ...
            fill="#eee"
            fontSize={10}
          >{`${p.nodeId} ${Math.round(p.latencyMs)}ms`}</text>
        </g>
      ))}
    </svg>
  );
