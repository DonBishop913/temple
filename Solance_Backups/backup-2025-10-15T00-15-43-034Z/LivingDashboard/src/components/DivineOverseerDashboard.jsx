...
export default function DivineOverseerDashboard() {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/api/metrics').then(r => r.json()).then(setMetrics)
    fetch('http://localhost:3000/api/alerts').then(r => r.json()).then(setAlerts)
  }, [])
          ...
          <ul className="space-y-2">
            {alerts.map(a => (
              <li key={a.id} className="flex items-center gap-2">
                <span className={`inline-block rounded px-2 py-1 text-xs ${a.level === 'warn' ? 'bg-yellow-200' : 'bg-blue-200'}`}>{a.level}</span>
                <span>{a.message}</span>
              </li>
            ))}
          </ul>
