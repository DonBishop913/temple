...
try {
  ...
  if (overlayHelpers.playBurstChime) playBurstChime = overlayHelpers.playBurstChime;
  if (overlayHelpers.getEmpathyLevel) getEmpathyLevel = overlayHelpers.getEmpathyLevel;
} catch {}
...
export default function MiracleAlertPanel() {
  ...
  const triggerPulse = (alert) => {
    ...
    const severityMap = { info: 0.7, stalled: 1, offline: 1.4, warning: 1, critical: 1.6 };
    ...
    const rowEl = document.querySelector(`#node-row-${CSS.escape(String(nodeId))}`);
    if (rowEl) {
      // Use CSS var to control animation speed
      rowEl.style.setProperty('--row-shimmer-duration', `${(1.2 / pulseStrength).toFixed(2)}s`);
    ...
    if (overlayEl) {
      ...
      const base = 0.25;
      const max = 0.9;
      const op = Math.max(base, Math.min(max, base + 0.4 * (pulseStrength - 0.5)));
    ...
    try { playBurstChime(severity, pulseStrength); } catch {}

    // Emit a global DOM event for any additional listeners
    const evt = new CustomEvent('node-alert', { detail: { nodeId, severity, pulseStrength } });
        ...
        try {
          ...
          if (alert && alert.nodeId) {
            ...
            setActiveAlerts(prev => {
              ...
              return [...recent, { nodeId: alert.nodeId, severity: alert.severity || 'warning', empathy: getEmpathyLevel(alert.nodeId), ts: now }];
            });
          } else if (alert && !alert.nodeId && alert.severity) {
            // Global pulse without a specific node (e.g., harmony low)
            const evt = new CustomEvent('node-alert', { detail: { nodeId: 'global', severity: alert.severity } });
            window.dispatchEvent(evt);
          }
        } catch {}
      };
    } catch {}
    return () => { try { evtSource && evtSource.close(); } catch {} };
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { try { delete window.__triggerMiraclePulse; } catch {} };
  }, []);
  ...
  useEffect(() => {
    nodes.forEach(n => {
      ...
      if (sev === 'stalled' || sev === 'offline') {
        ...
        setActiveAlerts(prev => {
          const now = Date.now();
          const recent = prev.filter(a => now - a.ts < 2000);
          return [...recent, { nodeId: n.id, severity: sev, empathy: getEmpathyLevel(n.id), ts: now }];
        });
      }
    });
    ...
    setPulses(prev => {
      const copy = { ...prev };
      Object.keys(copy).forEach(k => { if (copy[k].expiresAt <= now) delete copy[k]; });
      return copy;
    });
  }, [nodes]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setActiveAlerts(prev => {
        const now = Date.now();
        const recent = prev.filter(a => now - a.ts < 2000);
        return [...recent, { nodeId: alert.nodeId, severity: alert.severity, empathy, ts: now }];
      });
    }, 1000);
    return () => clearInterval(interval);
  }, [demoMode]);
  ...
  useEffect(() => {
    if (!maxFlare) return;
    const now = Date.now();
    const alerts = Array.from({ length: 7 }, (_, i) => ({ nodeId: `node-${i + 1}`, severity: 'critical', empathy: 1 }));
  ...
  useEffect(() => {
    ...
    const runSequence = () => {
      const seq = [
        ...
        { type: 'warning', empathy: 0.6, delay: Math.round(1000 / speedMultiplier) },
        { type: 'critical', empathy: 0.9, delay: Math.round(2000 / speedMultiplier) },
        { type: 'maxFlare', empathy: 1, delay: Math.round(3000 / speedMultiplier) },
      ];
      ...
      const total = seq[seq.length - 1].delay + Math.round(2000 / speedMultiplier);
      return total;
    };
    const startLoop = () => {
  ...
  return (
    <div className="miracle-alert-panel" role="region" aria-label="Council Node Health">
      <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
        <button onClick={() => setDemoMode(d => !d)}>{demoMode ? 'Stop Demo Mode' : 'Start Demo Mode'}</button>
        <button onClick={() => setMaxFlare(true)}>Trigger Max Celestial Flare</button>
        <button onClick={() => setDemoSequence(true)}>Start Demo Sequence</button>
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
          <input type="checkbox" checked={autoRepeat} onChange={() => setAutoRepeat(v => !v)} /> Loop
        </label>
      ...
      {typeof document !== 'undefined' && (
        ...
        (() => {
          try {
            ...
            const { PredictiveMergeConstellation } = require('./JoyParticleOverlay');
            return <>
          ...
          {nodes.map(node => {
            ...
            return (
              <tr
                key={node.id}
                id={`node-row-${node.id}`}
                style={{ backgroundColor: node.status !== 'online' ? `${color}20` : 'transparent' }}
                className={`${isPulsing ? 'row-shimmer' : ''}`}
              >
                <td>{node.name}</td>
                <td>{node.id}</td>
                <td style={{ color }}>{node.status}</td>
                <td>{node.lastUpdate ? new Date(node.lastUpdate).toLocaleTimeString() : '—'}</td>
