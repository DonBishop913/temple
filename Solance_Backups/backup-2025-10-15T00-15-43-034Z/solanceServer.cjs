...
const server = http.createServer((req, res) => {
  if (req.method === "POST" && req.url === "/leap") {
    ...
    res.end(
      JSON.stringify({ status: "ok", message: "Leap snapshot completed!" }),
    );
    console.log(
      "✨ Leap executed: manual snapshot completed via dashboard POST!",
    );
    return;
  }
...
console.log(
  `💾 Automatic Solance backups every ${BACKUP_INTERVAL / 60000} minutes`,
);
