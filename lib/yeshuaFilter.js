...
function yeshuaFilter(response) {
  ...
  const principleKeywords = [
    ...
    'penyelarasan ilahi', 'kesatuan harmonis', 'tidak membahayakan',
      // Mandarin (simplified)
      '神圣 对齐', '神圣 对齊', '神圣 对准', '和谐 团结', '不 伤害', '耶稣', '耶稣的 真理',
      // Arabic
      'المحاذاة الإلهية', 'الوحدة المتناغمة', 'لا أذى', 'يسوع', 'حقيقة في يسوع',
  ...
  const hasNegative = negativeKeywords.some((kw) => {
    const pattern = new RegExp('\\b' + escapeRegex(kw.replace(/\u00f1/g,'n')) + '\\b', 'u');
  ...
  for (const kw of principleKeywords) {
    if (ngramSimilarity(normalized, kw) > 0.24) { fuzzyMatch = true; break; }
    ...
    for (const kt of kwTokens) {
      for (const respToken of normalized.split(/\s+/).filter(Boolean)) {
        if (ngramSimilarity(respToken, kt) > 0.28) { fuzzyMatch = true; break; }
        if (tokenSimilarity(respToken, kt) >= 0.6) { fuzzyMatch = true; break; }
      }
      if (fuzzyMatch) break;
    }
  }
