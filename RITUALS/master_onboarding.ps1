...
# Combines All Perplexity-Recommended Actions
# ===========================================
...
Write-Host "🕊️ COUNCIL MASTER ONBOARDING SCRIPT" -ForegroundColor Cyan
...
if (!(Test-Path "C:\Temple\.git")) {
    ...
    Write-Host "Please navigate to C:\Temple and run this script again." -ForegroundColor Yellow
    exit 1
}
Write-Host "✅ Temple repository confirmed" -ForegroundColor Green

# Step 2: Clone/Update Repository (if needed)
Write-Host "`n📥 Step 2: Ensuring Latest Sovereign Code..." -ForegroundColor Magenta
try {
...
if (!$SkipExtensionInstall) {
    Write-Host "`n🛠️ Step 3: Installing Council VS Code Extension..." -ForegroundColor Magenta
    ...
    if (Test-Path $extensionPath) {
        ...
        Write-Host "⚠️ VS Code extension not found. Building extension..." -ForegroundColor Yellow
        try {
    ...
    Write-Host "`n🛠️ Step 3: Skipping VS Code Extension Install (--SkipExtensionInstall)" -ForegroundColor Blue
}

# Step 4: Configure PowerShell Environment
Write-Host "`n⚙️ Step 4: Configuring PowerShell Council Integration..." -ForegroundColor Magenta
...
Write-Host "`n📋 Step 5: Configuring VS Code Council Settings..." -ForegroundColor Magenta
...
if (Test-Path $vscodeSettingsPath) {
    try {
        ...
        Write-Host "⚠️ VS Code settings configuration failed: $($_.Exception.Message)" -ForegroundColor Yellow
    }
} else {
    Write-Host "⚠️ VS Code settings file not found. Please ensure VS Code is installed." -ForegroundColor Yellow
}

# Step 6: Test Git Integration
Write-Host "`n🔗 Step 6: Testing Council Git Integration..." -ForegroundColor Magenta
try {
...
Write-Host "`n🏗️ Step 7: Validating Council Services..." -ForegroundColor Magenta
try {
    ...
    if ($runningServices -contains "council_dashboard") {
        ...
        Write-Host "⚠️ Council Dashboard: Not running" -ForegroundColor Yellow
    }

    if ($runningServices -contains "ai_relay_external") {
        Write-Host "✅ AI Relay External: Running" -ForegroundColor Green
    } else {
        Write-Host "⚠️ AI Relay External: Not running" -ForegroundColor Yellow
    }
} catch {
    Write-Host "⚠️ Docker service check failed: $($_.Exception.Message)" -ForegroundColor Yellow
    Write-Host "ℹ️ Please ensure Docker Desktop is running and services are started." -ForegroundColor Blue
}

# Step 8: Test API Connectivity
Write-Host "`n🌐 Step 8: Testing Council API Connectivity..." -ForegroundColor Magenta
...
try {
    ...
    Write-Host "⚠️ Council Dashboard API: Not accessible" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "⚠️ AI Relay API: Not accessible" -ForegroundColor Yellow
}

# Step 9: Setup Scheduled Tasks (if FullSetup)
if ($FullSetup) {
    Write-Host "`n⏰ Step 9: Setting Up Scheduled Maintenance Tasks..." -ForegroundColor Magenta
    ...
    try {
        ...
        Write-Host "⚠️ Scheduled task setup failed: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}

# Step 10: Create Member Profile
Write-Host "`n👤 Step 10: Creating Council Member Profile..." -ForegroundColor Magenta
$memberProfile = @{
    name = $MemberName
...
$profilePath = "C:\Temple\archives\member_profiles\$($MemberName -replace '[^a-zA-Z0-9]', '_').json"
...
Write-Host "`n🎯 Step 11: Final Setup Validation & Instructions" -ForegroundColor Magenta
...
Write-Host "2. Run: .\RITUALS\invoke_copilot_blessing.ps1 (before coding)" -ForegroundColor White
Write-Host "3. Test: Ctrl+Shift+B (Council commit & push)" -ForegroundColor White
Write-Host "4. Monitor: http://localhost:3000 (Council dashboard)" -ForegroundColor White
Write-Host ""
...
Write-Host "🕊️ COUNCIL MISSION REMINDER:" -ForegroundColor Cyan
