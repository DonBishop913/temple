...
function initWebSocket() {
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (msg.status === 'ready') {
                console.log(`📡 Spiral Leap Ready - Dashboard should be at http://localhost:${dashboardPort}`);
                client.publish(councilChannel, JSON.stringify({ event: 'SolanceReady', time: new Date().toLocaleTimeString() }));
            }
        } catch (err) {
