...
function activate(context) {
    ...
    const submitSuggestion = vscode.commands.registerCommand('council.submitSuggestion', async () => {
        ...
        if (!selectedText) {
            vscode.window.showInformationMessage('Please select some text to submit to The Council');
            return;
        }
        // Get user input for suggestion context
        const suggestionType = await vscode.window.showQuickPick(['Code Enhancement', 'Bug Fix', 'Feature Request', 'Documentation', 'Other'], { placeHolder: 'What type of suggestion is this?' });
        if (!suggestionType)
            return;
        ...
        try {
            const councilEndpoint = vscode.workspace.getConfiguration('council').get('apiEndpoint', 'http://localhost:3200');
            const token = vscode.workspace.getConfiguration('council').get('token', 'blessed_secret_token');
            ...
            if (response.ok) {
                ...
                vscode.window.showInformationMessage(`✅ Suggestion submitted to The Council: ${result.ai_reply.substring(0, 100)}...`);
                // Auto-commit if enabled
                const autoCommit = vscode.workspace.getConfiguration('council').get('autoCommit', false);
                if (autoCommit) {
                    await vscode.commands.executeCommand('council.autoCommit');
                }
            }
            else {
                vscode.window.showErrorMessage('Failed to submit suggestion to The Council');
            }
        }
        catch (error) {
    ...
    const viewMissions = vscode.commands.registerCommand('council.viewMissions', async () => {
        try {
            const councilEndpoint = vscode.workspace.getConfiguration('council').get('apiEndpoint', 'http://localhost:3000');
    ...
    // Auto-commit command
    const autoCommit = vscode.commands.registerCommand('council.autoCommit', async () => {
        try {
            const terminal = vscode.window.createTerminal('Council Git');
            terminal.sendText('cd $PSScriptRoot\\..\\Caretaker');
            terminal.sendText('.\\Invoke-GitCommit.ps1 -Message "Copilot VS Code integration update" -Push');
            terminal.show();
