...
const processes = [
  ...
  { name: "AutonomousTriggers", command: "node TemplePC-AutonomousTriggers.js" },
  { name: "PredictiveForesight", command: "node TemplePC-PredictiveForesight.js" },
];
...
function startProcess(proc) {
  ...
  child.on("exit", (code, signal) => {
    console.warn(`⚠️ ${proc.name} exited with code ${code} signal ${signal}. Restarting in 5s...`);
...
watchDirs.forEach(dir => {
  fs.watch(dir, { recursive: true }, (eventType, filename) => {
    if (filename.endsWith(".js") || filename.endsWith(".jsx")) {
      console.log(`✨ Detected change in ${filename}, triggering auto-refresh.`);
      // Optional: send a signal or restart affected script
    }
  });
});

console.log("🛡️ Sovereign Continuity Layer active: all subsystems protected, 24/7 autonomous operation.");
