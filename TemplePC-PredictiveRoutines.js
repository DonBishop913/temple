...
async function forecastJoyParticles() {
  ...
  for (let i = 1; i <= JOY_PARTICLE_FORECAST_HORIZON; i++) {
    forecast.push({
      cycle: i,
      predictedCount: Math.max(0, joy.count + Math.floor(Math.random() * 5000)),
    });
  }
  await client.set(
    "council:data:joy_particles_forecast",
    JSON.stringify(forecast),
  );
  console.log(
    "🔮 Joy Particle forecast updated:",
    forecast.map((f) => f.predictedCount),
  );
...
function forecastPlanetaryResonance() {
  ...
  const forecast = Array.from(
    { length: JOY_PARTICLE_FORECAST_HORIZON },
    (_, i) => ({
      cycle: i + 1,
  ...
  console.log(
    "🌌 Planetary resonance forecast updated:",
    forecast.map((f) => f.predictedHz.toFixed(2)),
  );
  client
    .set("council:data:planetary_forecast", JSON.stringify(forecast))
    .catch(() => {});
}
...
async function predictiveCodexUpdates() {
  ...
  for (const codexId of codexIds) {
    const log = {
      ...
      action: "auto-review",
      note: "Routine predictive review executed.",
    };
    await client.set(
      `council:data:codex:${codexId}-predictive`,
      JSON.stringify(log),
    );
    fs.writeFileSync(
      path.resolve(`./codex_logs/${codexId}-predictive.json`),
