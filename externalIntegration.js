...
class ExternalIntegrationService {
  ...
  loadConfiguration() {
    try {
      ...
      console.error(
        "❌ Failed to load external services config:",
        error.message,
      );
      this.services = [];
    }
  ...
  handleSyncFailure(service, error) {
    ...
    if (service.name === "CouncilRelay") {
      console.log(
        "🛡️ Activating sovereign fallback mode for Council operations",
      );
    }
  }
  ...
  startSyncCycles() {
    ...
    this.services.forEach((service) => {
      if (service.enabled && service.sync_interval) {
        ...
        console.log(
          `⏰ Scheduled ${service.name} sync every ${service.sync_interval / 1000}s`,
        );
      }
    });
  }
