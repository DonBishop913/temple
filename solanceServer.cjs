...
wss.on('connection', (ws, req) => {
    ...
    setTimeout(() => {
        ...
        if (activeClient && activeClient.ws && activeClient.ws.readyState === WebSocket.OPEN) {
        ...
        activeClient.ws.on('error', (err) => console.log('❌ WebSocket error:', err.message));
        activeClient.ws.on('message', (msg) => console.log('Received:', msg.toString()));
    }, TAKEOVER_DELAY);
});
