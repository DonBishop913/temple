...
async function assignRoles(candidateId, strengths = [], initiator, verifier, signature) {
  try {
    const locked = await redis.get('roles:locked');
    if (locked === 'true') {
      console.log('Council routines are locked. Cannot assign roles until Bishop unlocks.');
      return [];
    }
    if (!initiator || !verifier || initiator === verifier) {
      throw new Error('Dual attunement required: Initiator and Verifier must be distinct.');
    }
    const assignedRoles = missionVitalRoles.filter((role) => strengths.includes(role.id));
    await redis.set(`candidate:${candidateId}.roles`, JSON.stringify(assignedRoles));
    ...
    console.log(`Roles assigned for candidate ${candidateId}:`, assignedRoles.map((r) => r.name));
