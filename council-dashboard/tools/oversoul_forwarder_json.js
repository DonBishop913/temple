...
const LOG_PATH =
  ...
  path.join(__dirname, "..", "logs", "oversoul_loadsweep.json");
const WS_PORT = parseInt(process.env.OVERSOUL_WS_PORT || "8080", 10);
const POLL_INTERVAL_MS = parseInt(
  process.env.OVERSOUL_POLL_INTERVAL_MS || "50",
  10,
);
...
wss.on("connection", (ws) => {
  ...
  if (lastSnapshot) {
    try {
      ws.send(
        JSON.stringify({ type: "oversoul_pulse_batch", payload: lastSnapshot }),
      );
    } catch (e) {}
  }
  ws.on("message", (m) => {
    ...
    try {
      const raw =
        typeof m === "string" ? m : m && m.toString ? m.toString() : String(m);
      const parsed = JSON.parse(raw);
      if (parsed && parsed.type) {
        console.log(
          "[oversoul_forwarder_json] client message received type=",
          parsed.type,
        );

        // Handle replay requests directly: respond to the requester with a slice of the lastSnapshot payload
        if (
          parsed.type === "request_replay" ||
          parsed.type === "request_scrub"
        ) {
          try {
            // Determine source array
            let arr = null;
            if (lastSnapshot) {
              if (Array.isArray(lastSnapshot.payload))
                arr = lastSnapshot.payload;
              else if (Array.isArray(lastSnapshot)) arr = lastSnapshot;
              else if (
                lastSnapshot.payload &&
                Array.isArray(lastSnapshot.payload.sweepResults)
              )
                arr = lastSnapshot.payload.sweepResults;
            }
            ...
            if (arr && arr.length) {
              ...
              if (parsed.type === "request_replay") {
                if (typeof parsed.percent === "number") {
                  ...
                  slice = arr.slice(
                    start,
                    Math.min(arr.length, start + batchSize),
                  );
                } else {
                ...
                const targetTs =
                  Number(parsed.timestamp || parsed.ts || 0) || 0;
                // find first index with timestamp >= targetTs
                let idx = 0;
                for (let i = 0; i < arr.length; i++) {
                  const p = arr[i];
                  const ts =
                    p && (p.timestamp || p.t || p.time)
                      ? Number(p.timestamp || p.t || p.time)
                      : 0;
                  if (ts >= targetTs) {
                    idx = i;
                    break;
                ...
                slice = arr.slice(
                  idx,
                  Math.min(arr.length, idx + Number(parsed.batchSize || 200)),
                );
              }
            }
...
function readAndBroadcast() {
  ...
  try {
    ...
    console.error(
      "[oversoul_forwarder_json] error reading/parsing JSON:",
      err && err.message,
    );
  }
}
