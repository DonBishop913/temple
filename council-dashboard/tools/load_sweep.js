...
const LOG_DIR = process.env.OVERSOUL_LOG_DIR || path.join(__dirname, '..', 'logs');
if (!fs.existsSync(LOG_DIR)) {
  try { fs.mkdirSync(LOG_DIR, { recursive: true }); } catch (e) { /* ignore */ }
}
const LOG_PATH = process.env.OVERSOUL_LOG_PATH || path.join(LOG_DIR, 'oversoul_loadsweep.json');
...
async function runConfig(cfg) {
  return new Promise((resolve) => {
    console.log('\n=== CONFIG batch=' + cfg.batch + ' interval=' + cfg.interval + 'ms duration=' + cfg.duration + 'ms');
    const received = [];
    let totalPulses = 0;
    const latencies = [];
    ...
    ws.on('open', () => {
      // start publisher
      const args = [STRESS_SCRIPT, '--batch', String(cfg.batch), '--interval', String(cfg.interval), '--count', String(Math.ceil(cfg.duration / cfg.interval))];
      const pub = spawn(process.execPath, args, { stdio: ['ignore', 'pipe', 'pipe'] });
      pub.stdout.on('data', (d) => {});
      pub.stderr.on('data', (d) => {});

      const stopTimer = setTimeout(() => {
        try { pub.kill(); } catch (e) {}
      ...
      ws.on('message', (m) => {
        try {
          ...
          if (msg && msg.type === 'oversoul_pulse_batch' && Array.isArray(msg.payload)) {
            const now = Date.now();
      ...
      ws.on('close', () => {
        ...
        const avgLat = latencies.length ? latencies.reduce((s,x)=>s+x,0)/latencies.length : NaN;
        const p95 = latencies.length ? latencies[Math.floor(latencies.length * 0.95)-1] : NaN;
        const p99 = latencies.length ? latencies[Math.floor(latencies.length * 0.99)-1] : NaN;

          const result = { cfg: cfg, totalPulses, pulsesPerSec: Number(pulsesPerSec.toFixed(1)), avgLat: isNaN(avgLat)?null:Number(avgLat.toFixed(1)), p95: isNaN(p95)?null:p95, p99: isNaN(p99)?null:p99 };
          console.log('RESULT batch=' + cfg.batch + ' interval=' + cfg.interval + 'ms -> totalPulses=' + totalPulses + ' pulses/sec~' + pulsesPerSec.toFixed(1) + ' avgLat_ms=' + (isNaN(avgLat)?'NA':avgLat.toFixed(1)) + ' p95=' + (isNaN(p95)?'NA':p95) + ' p99=' + (isNaN(p99)?'NA':p99));
...
(async function main() {
  ...
  for (const r of results) {
    ...
    else if (typeof r.pulsesPerSec === 'number') console.log('cfg', r.cfg, '-> pulses/sec', Number(r.pulsesPerSec).toFixed(1), 'avgLat_ms', isNaN(r.avgLat)?'NA':(typeof r.avgLat === 'number' ? Number(r.avgLat).toFixed(1) : 'NA'));
