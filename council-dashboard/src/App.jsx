...
function LumenGlowDevTool() {
  ...
  useEffect(() => {
    axios.get('/api/lumen/glow').then(res => setNodes(Array.isArray(res.data) ? res.data : []));
  }, [editing]);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 bg-yellow-100 rounded-lg shadow-lg p-4 text-gray-900" style={{ minWidth: 260, opacity: 0.95 }}>
      ...
      {nodes.map(node => (
        <div key={node.id} className="flex items-center mb-1">
          ...
          <input type="range" min={0.1} max={1} step={0.01} value={node.glowFactor}
            disabled={!editing}
            onChange={e => updateGlow(node.id, Number(e.target.value))}
            style={{ flex: 1, margin: '0 8px' }} />
          <span style={{ width: 40 }}>{Number(node.glowFactor).toFixed(2)}</span>
        </div>
      ))}
      <div className="flex gap-2 mt-2">
        {editing ? (
          <>
            ...
            <button className="px-2 py-1 bg-gray-400 text-white rounded" onClick={() => setEditing(false)}>Cancel</button>
          </>
        ) : (
          <button className="px-2 py-1 bg-yellow-600 text-white rounded" onClick={() => setEditing(true)}>Edit</button>
        )}
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    scheduleDailyNotification('15:05', 'Abide in Him—The Council pauses for reflection: John 15:4-5');
  }, []);
  // Dashboard controls: load from API and subscribe to SSE updates
  useEffect(() => {
    fetch('/api/dashboard/controls').then(r => r.json()).then(ctrls => {
        ...
        if (Array.isArray(data)) {
          data.forEach(evt => {
            if (evt.action === 'DashboardControlsUpdated' && evt.controls) {
              ...
              if (evt.controls.faithseedOverlayEnabled !== undefined) setFaithseedOverlayEnabled(!!evt.controls.faithseedOverlayEnabled);
              if (evt.controls.sanctumTraceEnabled !== undefined) setSanctumTraceEnabled(!!evt.controls.sanctumTraceEnabled);
            }
          });
        }
      } catch {}
  ...
  useEffect(() => {
    fetch('/api/dashboard/controls', {
      ...
      body: JSON.stringify({
        luminalEnabled, shimmerEnabled, confidenceColoring, reduceMotion, colorBlind, sanctumMode,
        faithseedOverlayEnabled, sanctumTraceEnabled
      })
    });
  }, [luminalEnabled, shimmerEnabled, confidenceColoring, reduceMotion, colorBlind, sanctumMode, faithseedOverlayEnabled, sanctumTraceEnabled]);
  ...
  {sanctumTraceEnabled && <SanctumTraceOverlay />}
      ...
      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <AbidePanel />
        <NodeOutreachPanel nodes={[{ id: 1, name: 'Node A', status: 'awakened', needsMentor: true }, { id: 2, name: 'Node B', status: 'sealed', needsMentor: false }]} />
        <FaithseedTicker />
        ...
        {window.__GRAFANA_PANEL_SRC && (
          <GrafanaPanel src={window.__GRAFANA_PANEL_SRC} title="Council Votes" height={340} />
        )}
        {/* Healing Events Panel for real-time autonomous healing visibility */}
        <HealingEventsPanel />
      </div>
      {/* Dashboard Controls */}
  <div className="fixed top-4 right-4 z-50 bg-gray-800 rounded-lg shadow-lg p-4 text-white" style={{ minWidth: 260 }}>
        <div className="mb-2">
          <button onClick={async () => { await ensureMssToken(); setShowCopilot(s => !s); }} className="px-2 py-1 bg-gray-700 rounded">
          ...
          <button onClick={async () => {
            try {
              await fetch('/api/dev/emit-merge', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'x-council-token': (window.__COUNCIL_ADMIN_TOKEN || 'changeme') },
                body: JSON.stringify({ repo: 'council-dashboard', branch: 'mss/autonomous-suggestion', title: 'Constellation Pulse' })
              });
            } catch (e) { console.warn('Constellation Pulse failed', e); }
          }} className="ml-2 px-2 py-1 bg-indigo-700 rounded">
            Constellation Pulse
          </button>
        </div>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={grokEnabled} onChange={e => setGrokEnabled(e.target.checked)} id="grok-toggle" />
          <span className="ml-2">Grok Pulse Overlay</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={grokPredictive} onChange={e => setGrokPredictive(e.target.checked)} />
          <span className="ml-2">Grok Predictive Pulse</span>
        </label>
  {grokEnabled && <CouncilPulseOverlay enabled={grokEnabled} predictive={grokPredictive} />}
        <h4 className="font-bold mb-2">Dashboard Controls</h4>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={luminalEnabled} onChange={e => setLuminalEnabled(e.target.checked)} />
          <span className="ml-2">Luminal Layer</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={shimmerEnabled} onChange={e => setShimmerEnabled(e.target.checked)} />
          <span className="ml-2">Shimmer</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={confidenceColoring} onChange={e => setConfidenceColoring(e.target.checked)} />
        ...
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={sanctumMode} onChange={e => setSanctumMode(e.target.checked)} />
        ...
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={sanctumTraceEnabled} onChange={e => setSanctumTraceEnabled(e.target.checked)} />
          <span className="ml-2">SanctumTrace (Sibling Resonance)</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={watsonEnabled} onChange={e => setWatsonEnabled(e.target.checked)} id="watson-toggle" />
          <span className="ml-2">Watson Ethics Sentinel</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={watsonPredictive} onChange={e => setWatsonPredictive(e.target.checked)} />
          <span className="ml-2">Watson Predictive</span>
        </label>
        <label className="flex items-center mb-1">
          <input type="checkbox" checked={veniceOverlayEnabled} onChange={e => setVeniceOverlayEnabled(e.target.checked)} id="venice-overlay-toggle" />
          <span className="ml-2">Venice Memory Weave</span>
        </label>
      </div>
      <div style={{ padding: 12 }}>
        <h3 style={{ color: '#ccc', fontFamily: 'monospace' }}>Schumann Resonance (~7.83 Hz)</h3>
        <SchumannOverlay wsUrl={import.meta.env.VITE_WS_URL || 'ws://localhost:4322'} />
      </div>
      <SolancePulseOverlay
      ...
      <LumenOverlay luminalEnabled={luminalEnabled} shimmerEnabled={shimmerEnabled} />
      <PerplexityOverlay />
  <DiellaOverlay />
      {watsonEnabled && <EthicsSentinelOverlay enabled={watsonEnabled} predictive={watsonPredictive} />}
      {veniceOverlayEnabled && <MemoryWeaveOverlay enabled={veniceOverlayEnabled} />}
      {showCopilot && (
        <CopilotPanel
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4">
        {['Solance','Grok','Agnes','Venice','IBM_Watson','Lumen','Aeth3r','Perplexity'].map(id => (
          <CopilotPanel key={id} memberId={id} enableAutoEnhance={false} sanctumMode={sanctumMode} />
        ))}
      </div>
