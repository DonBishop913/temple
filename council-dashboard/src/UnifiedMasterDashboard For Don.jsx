...
export default function UnifiedMasterDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setQueueStatus(prev => ({ ...prev, lastAttempt: new Date().toLocaleTimeString() }));
      try {
        const [ripplesRes, energyRes, overrideRes, harmonyRes, feedRes, nodesRes] = await Promise.all([
        ...
        setLinkPulses(prev => {
          const pulse = { id: Date.now(), magnitude: ripplesRes.ripples, timestamp: ripplesRes.timestamp };
          const next = [...prev, pulse];
          return next.slice(-100); // cap pulses
        });

        // Energy flow -> spiral metrics
        setSpiralMetrics({ flowRate: Number(energyRes.flowRate), unit: energyRes.unit, timestamp: energyRes.timestamp });

        // Override pulse -> alert banner
        if (overrideRes.active) {
          setAlertBanner({ status: 'orange', message: `OverridePulse active from ${overrideRes.source}` });
        } else {
        ...
        setChronicleEntries(prev => [...prev, { timestamp: new Date(feedRes.timestamp).toLocaleTimeString(), entry: feedRes.message }].slice(-200));
        ...
        setNodes(prev => {
          ...
          return prev.map((n, idx) => {
            const src = apiNodes[idx] || apiNodes.find(x => x.name === n.name);
            const engagement = src ? Number(src.engagement) : Math.floor(Math.random() * 100);
            const resonance = Math.max(50, Math.min(432, Math.round(engagement * 3))); // scale
            return { ...n, resonance, schumann: engagement };
          });
        });

        // Queue status
        setQueueStatus(prev => ({ ...prev, count: Math.floor(Math.random() * 10) }));
        ...
        if (councilChartRef.current && Chart) {
          ...
          if (!councilChartRef.current._chart) {
            ...
            councilChartRef.current._chart = new Chart(ctx, {
              type: 'line',
              data: { labels: [], datasets: [{ label: 'Harmony Score', data: [], borderColor: '#4D79FF' }] },
        ...
        setLogs(prev => [...prev, { timestamp: new Date().toLocaleTimeString(), message: `Fetch error: ${err?.message || err}` }].slice(-200));
      }
    };
