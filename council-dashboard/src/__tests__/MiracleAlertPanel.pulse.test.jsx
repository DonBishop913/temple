  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (overlayRef.current) {
        ...
        setTimeout(
          () =>
            overlayRef.current && overlayRef.current.classList.remove("glow"),
          20,
        );
      }
    ...
    <div>
      ...
      <table>
        <tbody>
          <tr
            id={`node-${nodeId}`}
            data-testid={`row-${nodeId}`}
            className={isShimmer ? "row-shimmer" : ""}
          >
            <td>Aletheia</td>
          </tr>
        </tbody>
...
describe("MiracleAlertPanel shimmer and overlay pulses", () => {
  test("activates shimmer class on node alert and then clears quickly", async () => {
    ...
    const evt = new CustomEvent("node-alert", {
      detail: { nodeId: "node-002", severity: "stalled" },
    });
  ...
  test("triggers overlay pulse once per event", async () => {
    ...
    window.dispatchEvent(
      new CustomEvent("node-alert", {
        detail: { nodeId: "node-002", severity: "offline" },
      }),
    );
