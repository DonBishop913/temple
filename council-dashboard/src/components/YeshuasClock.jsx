...
export default function YeshuasClock() {
  ...
  const avgEngagement = nodes.length
    ? nodes.reduce((a, n) => a + (n.engagement || 0), 0) / nodes.length
    : 0.8;
  ...
  const params = blend({
    ...
    starlinkQuality,
    joy,
    harmonicSensitivity: harmonicEnabled ? harmonicSensitivity : 0,
  });
  ...
  const planets =
    grok.planets && grok.planets.length ? grok.planets : state.planetary || [];
  ...
  return (
    <div
      style={{
        ...
        width: "100%",
        height: 400,
        background: "radial-gradient(ellipse at center, #0a0f1f 0%, #000 70%)",
        color: "#cde",
      }}
    >
      <h2 style={{ textAlign: "center" }}>Yeshua's Clock</h2>
      {/* SpaceX Overlay Replay Controls */}
      <YeshuasClockReplayControls
      ...
      <PlanetOrbits
        ...
        glowIntensity={params.glowIntensity + (params.orbitGlowMicro || 0)}
      />
      {/* Comet trail overlay (Grok missions + Council heartbeat, or SpaceX replay) */}
      <Comet3IOverlay
        data={
          isReplaying && replayFrames[replayIdx]
            ? replayFrames[replayIdx].missions
            : missions
        }
      ...
      <SpectralWebOverlay
        nodes={nodes}
        planets={planets}
        energy={{ ...(state.energy || {}), webPulse: params.webPulse }}
      />
      ...
      <GlyphstreamOverlay
        ...
        resonanceData={nodes.map((n, i) => ({
          x: 100 + i * 40,
          y: 200 + Math.sin(i) * 30,
          strength: (n.engagement || 50) / 100,
        }))}
      ...
      >
        Pulse: {params.pulseCadence.toFixed(2)} breaths/s
      </div>
    </div>
  );
}
