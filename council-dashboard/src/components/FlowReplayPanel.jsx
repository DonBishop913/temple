...
export default function FlowReplayPanel() {
  ...
  useEffect(() => {
    ...
    try {
      ws.current = new WebSocket((process.env.REACT_APP_REPLAY_WS || 'ws://localhost:4323'))
    } catch (e) {
      console.warn('FlowReplayPanel: websocket init failed', e && e.message)
      return
    }

    ws.current.onopen = () => console.log('FlowReplayPanel: connected to replay WS')
    ws.current.onclose = () => console.log('FlowReplayPanel: replay WS closed')
    ws.current.onerror = (err) => console.warn('FlowReplayPanel: ws error', err)
  ...
  const handleSeek = (index) => {
    ...
    } catch (e) { console.warn('seek send failed', e && e.message) }
  }

  const handleAddAnnotation = () => {
    const note = window.prompt('Enter annotation for this moment:')
    if (!note) return
    setAnnotations(prev => [...prev, { index: currentIndex, note, at: new Date().toISOString() }])
  }

  const currentEvent = events[currentIndex] || null

  return (
    <div style={{ padding: 20, fontFamily: 'Helvetica, Arial, sans-serif', background: '#E0F7FA', borderRadius: 8 }}>
      <h3 style={{ marginTop: 0 }}>🌟 Flow Replay Panel</h3>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
        <button onClick={handlePlayPause}>{playing ? '⏸ Pause' : '▶️ Play'}</button>
        <button onClick={handleAddAnnotation}>📝 Add Annotation</button>
        <div style={{ marginLeft: 'auto', fontSize: 12, color: '#333' }}>{events.length} events</div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ width: 160, fontSize: 12 }}>
          {currentEvent && (currentEvent.timestamp || currentEvent.at || currentEvent.ts)
            ? new Date((currentEvent.timestamp || currentEvent.at || currentEvent.ts)).toLocaleString()
            : '--:--'}
        </div>
      </div>

      <div style={{ marginTop: 12, background: '#fff', padding: 12, borderRadius: 6, maxHeight: 320, overflowY: 'auto' }}>
        <pre style={{ margin: 0, fontSize: 12 }}>{currentEvent ? JSON.stringify(currentEvent, null, 2) : 'No event selected'}</pre>
      </div>
      ...
      <div style={{ marginTop: 12 }}>
        ...
        <ul>
          {annotations.map((a, i) => (
            <li key={i}>[{a.index}] {a.note} <span style={{ color: '#666', marginLeft: 8 }}>{new Date(a.at).toLocaleString()}</span></li>
          ))}
        </ul>
      </div>
    </div>
