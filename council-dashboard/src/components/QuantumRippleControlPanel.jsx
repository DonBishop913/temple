...
export default function QuantumRippleControlPanel({
  quantumRippleField,
  sendHighlight,
  sendScrub,
}) {
  ...
  const togglePause = () => {
    ...
    setPaused(next);
    try {
      if (quantumRippleField && quantumRippleField.pause)
        quantumRippleField.pause(next);
      else if (
        quantumRippleField &&
        quantumRippleField.stateRef &&
        quantumRippleField.stateRef.current &&
        quantumRippleField.stateRef.current.api &&
        quantumRippleField.stateRef.current.api.pause
      ) {
  ...
  const handleScrub = (e) => {
    ...
    setScrubValue(val);
    try {
      // request server replay at percent
  ...
  const resetHighlights = () => {
    try {
      if (quantumRippleField && quantumRippleField.resetHighlights)
        quantumRippleField.resetHighlights();
      else if (
        quantumRippleField &&
        quantumRippleField.stateRef &&
        quantumRippleField.stateRef.current &&
  ...
  const handleBurstHighlight = () => {
    try {
      let ids = [];
      if (quantumRippleField && quantumRippleField.getRecentPulseIds)
        ids = quantumRippleField.getRecentPulseIds(burstCount);
      else if (
        quantumRippleField &&
        quantumRippleField.stateRef &&
        quantumRippleField.stateRef.current &&
        quantumRippleField.stateRef.current.api &&
        quantumRippleField.stateRef.current.api.getRecentPulseIds
      )
        ids =
          quantumRippleField.stateRef.current.api.getRecentPulseIds(burstCount);
      if (!ids || ids.length === 0) return;
      // highlight locally via renderer api if available
      if (quantumRippleField && quantumRippleField.highlightPulses)
        quantumRippleField.highlightPulses(ids, 1500);
      else if (
        quantumRippleField &&
        quantumRippleField.stateRef &&
        quantumRippleField.stateRef.current &&
        quantumRippleField.stateRef.current.api &&
        quantumRippleField.stateRef.current.api.highlightPulses
      ) {
      ...
      console.warn("burst highlight failed", e);
    }
  };
  ...
  return (
    ...
    >
      <div style={{ marginBottom: "8px" }}>
        <strong>Quantum Ripple Control</strong>
      </div>
      <div>FPS: {fps}</div>
      <div>Pulses/sec: {pulsesPerSec}</div>
      <div>Highlighted: {highlightedCount}</div>
      <div style={{ marginTop: "8px" }}>
        <button onClick={togglePause} style={{ marginRight: "6px" }}>
          {paused ? "Resume" : "Pause"}
        </button>
      ...
      <div style={{ marginTop: "8px" }}>
        ...
        <div style={{ display: "flex", gap: "6px", marginTop: "6px" }}>
          <input
            ...
            value={scrubTs}
            onChange={(e) => setScrubTs(e.target.value)}
            style={{ flex: 1 }}
          />
          <button
            onClick={() => {
              try {
                const ts = Number(scrubTs);
                if (!isNaN(ts))
                  quantumRippleField &&
                    quantumRippleField.scrubToTimestamp &&
                    quantumRippleField.scrubToTimestamp(ts);
              } catch (e) {}
            }}
          >
            Scrub→TS
      ...
      <div style={{ marginTop: "8px" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          <input
            type="number"
            value={burstCount}
            onChange={(e) => setBurstCount(Number(e.target.value || 0))}
            style={{ width: "80px" }}
          />
