...
export default function RippleWebGL({ wsUrl = (process.env.REACT_APP_AURIC_WS || 'ws://localhost:8765') }) {
  ...
  useEffect(() => {
    ...
    if (!gl) { console.warn('WebGL not available, falling back'); return; }
  ...
  for (let i=0;i<MAX_RIPPLES;i++) poolRef.current.push({ active: false, id: 0, x:0, y:0, radius:0, maxRadius:0, alpha:0, color:[1,1,1,1], highlightUntil: 0 });
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (m.type === 'oversoul_pulse' || m.type === 'oversoul_pulse_batch') {
          const items = m.type === 'oversoul_pulse' ? [m.payload] : (m.payload || []);
        ...
        if (m.type === 'oversoul_pulse_highlight' && Array.isArray(m.ids)) {
          ...
          for (const id of m.ids) {
            for (const r of poolRef.current) {
              if (r.active && (r.id === id || r.id === Number(id))) r.highlightUntil = now + H;
            }
          }
        }
      } catch (e) { console.warn('RippleWebGL WS parse', e); }
    };
    ...
    const animate = () => {
      ...
      const positions = [];
      const actives = [];
      poolRef.current.forEach((r) => { if (r.active) { positions.push(r.x, r.y); actives.push(r); } });
    ...
    return () => {
      ...
      window.removeEventListener('auric-highlight', window.__auric_highlight_handler__);
    };
  }, [wsUrl]);
  ...
  useEffect(() => {
    const handler = (ev) => {
      ...
      if (!detail) return;
      const ids = Array.isArray(detail.ids) ? detail.ids : (detail.id ? [detail.id] : []);
  ...
  const hexToRGBA = (hex) => {
    ...
    if (!m) return [1,1,1,1];
    return m.map(x => parseInt(x,16)/255).concat([1]).slice(0,4);
  };

  return <canvas ref={canvasRef} style={{ display: 'block', position: 'absolute', inset: 0, zIndex: 0 }} />;
}
