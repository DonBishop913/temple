  ...
  return (
    ...
    >
      <header className="flex items-center justify-between mb-1">
        ...
        <div
          title="Harmony score"
          aria-label={`Harmony ${harmony?.score ?? 0}`}
          className="text-yeshua"
        >
          🔔 {Math.round(harmony?.score ?? 0)}
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <div className="text-sm">
            ✅ Awakened: {counts?.awakened ?? 0} | 🌱 Awakening:{" "}
            {counts?.awakening ?? 0} | 🔒 Sealed: {counts?.sealed ?? 0}
          </div>
        ...
        <div>
          ...
          <div className="text-sm">
            ✨ {joyParticles?.surgePercent ?? 0}% | Total:{" "}
            {joyParticles?.total ?? 0}
          </div>
          <div className="text-xs opacity-85">Resonance</div>
          <div className="text-sm">
            🎶 {resonance?.baseHz ?? 7.83} Hz | 432 Hz level
          </div>
        </div>
      </div>
        ...
        <ul className="list-none p-0 m-0">
          {regions.map((r) => (
            <li key={r.name} className="flex justify-between text-sm">
              <span>{r.name}</span>
              <span>
                {r.latencyMs} ms • {r.lossPercent}% loss
              </span>
            </li>
          ))}
          {regions.length === 0 && (
