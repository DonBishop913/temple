...
export default function SpectralWebOverlay({
  nodes = [],
  planets = [],
  energy = { flowRate: 0 },
}) {
  ...
  const cx = width / 2,
    cy = height / 2;
  const energyAlpha = Math.min(
    0.8,
    0.2 + (Number(energy.flowRate || 0) / 10) * 0.6,
  );
    ...
    >
      {planetPoints.map((p, i) => (
        <circle
          ...
          cy={p.y}
          r={4}
          fill={`rgba(100,200,255,${energyAlpha})`}
        />
      ))}
      {nodePoints.map((n, i) => (
        <circle
          ...
          cy={n.y}
          r={3}
          fill={`rgba(255,255,255,${energyAlpha})`}
        />
      ))}
      {planetPoints.map((p, i) =>
        nodePoints.map((n, j) => (
          <line
            ...
            x2={n.x}
            y2={n.y}
            stroke={`rgba(135,206,250,${energyAlpha * 0.6})`}
            strokeWidth={1}
          />
        )),
      )}
