...
const QuantumRippleField = forwardRef(function QuantumRippleField({ wsUrl = (process.env.REACT_APP_AURIC_WS || 'ws://localhost:8080'), maxPulses = 10000, fadeFactor = 0.96 }, ref) {
  ...
  // stats
  const statsRef = useRef({ frames: 0, lastTS: Date.now(), fps: 0, pulsesReceived: 0, pulsesThisSecond: 0, lastPulseSecTS: Date.now() });
  ...
  useEffect(() => {
    ...
    function setCanvasSize() {
      ...
      if (prog && gl) try { gl.uniform2f(u_resolution, canvas.width, canvas.height); } catch (e) {}
    }
    setCanvasSize();
    ...
    if (!gl) {
      // Canvas2D fallback
      console.warn('QuantumRippleField: WebGL2 not supported in this browser. Falling back to 2D canvas.');
      ...
      function draw2D() {
        ...
        for (const p of q) {
          replayBufferRef.current.push({ id: p.id, x: p.x, y: p.y, radius: p.radius || 4, color: p.color, amplitude: p.amplitude, timestamp: p.timestamp || Date.now() });
        ...
        for (let i = start; i < buf.length; i++) {
          ...
          ctx.arc(p.x * canvas.clientWidth, p.y * canvas.clientHeight, p.radius || 3, 0, Math.PI * 2);
          ctx.fill();
        }
        ...
        if (now - statsRef.current.lastTS > 500) {
          statsRef.current.fps = Math.round((statsRef.current.frames * 1000) / (now - statsRef.current.lastTS));
          statsRef.current.frames = 0; statsRef.current.lastTS = now;
        }
        // pulses/sec decay
        if (now - statsRef.current.lastPulseSecTS > 1000) { statsRef.current.pulsesThisSecond = 0; statsRef.current.lastPulseSecTS = now; }
      ...
      stateRef.current.api = Object.assign(stateRef.current.api || {}, {
        addPulses: (arr) => { for (const p of arr) pulseQueueRef.current.push(p); },
        getStats: () => ({ fps: statsRef.current.fps, pulsesReceived: statsRef.current.pulsesReceived, pulsesThisSecond: statsRef.current.pulsesThisSecond }),
      });
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, MAX_HIGHLIGHTS, 1, 0, gl.RGBA, gl.UNSIGNED_BYTE, emptyTex);
  ...
  const ids = new Float32Array(maxPulses);
    ...
    function commitPulseToBuffers(p) {
      ...
      try { const oldId = ids[idx]; if (oldId) idToSlot.delete(oldId); } catch (e) {}
  ...
  try { idToSlot.set(ids[idx], idx); } catch (e) {}
      ...
      try {
        const copy = { id: ids[idx], x: x, y: y, radius: r, color: p.color, amplitude: p.amplitude, timestamp: p.timestamp || Date.now() };
        replayBufferRef.current.push(copy);
        if (replayBufferRef.current.length > REPLAY_LIMIT) replayBufferRef.current.splice(0, replayBufferRef.current.length - REPLAY_LIMIT);
      } catch (e) {}
      ...
      for (const cb of pulseListeners) {
        try { cb(p); } catch (e) { /* ignore listener errors */ }
      }
    }
    ...
    function updateHighlightTex(updates) {
      ...
      for (const u of updates) {
        if (!run) run = { start: u.slot, vals: new Uint8Array([u.r, u.g, u.b, u.a]) };
          ...
          // flush run
          gl.bindTexture(gl.TEXTURE_2D, highlightTex);
          gl.texSubImage2D(gl.TEXTURE_2D, 0, run.start, 0, run.vals.length/4, 1, gl.RGBA, gl.UNSIGNED_BYTE, run.vals);
          gl.bindTexture(gl.TEXTURE_2D, null);
    ...
    ws.addEventListener('open', () => {
      try { ws.send(JSON.stringify({ type: 'hello', wantReplay: true })); } catch (e) {}
      // request replay explicitly
      try { ws.send('REQUEST_REPLAY'); } catch (e) {}
    });
    ws.addEventListener('message', (ev) => {
      try {
        ...
        } else if (data && data.type === 'oversoul_pulse_batch' && Array.isArray(data.payload)) {
          data.payload.forEach(enqueuePulse);
        } else if (data && (data.type === 'replay' || data.type === 'replay_pulses') && Array.isArray(data.payload)) {
          // replace replay buffer with incoming and play them
          try { replayBufferRef.current = data.payload.map(px => ({ id: px.id || px.timestamp || Date.now(), x: px.x || 0, y: px.y || 0, radius: px.radius || 4, color: px.color, amplitude: px.amplitude, timestamp: px.timestamp || Date.now() })); } catch (e) {}
        ...
        } else if (data && data.type === 'oversoul_pulse_highlight' && Array.isArray(data.ids)) {
    ...
    function render() {
      ...
      if (activeCount > 0) {
        gl.bindBuffer(gl.ARRAY_BUFFER, posBuf);
        gl.bufferSubData(gl.ARRAY_BUFFER, 0, positions.subarray(0, activeCount*2));
        gl.bindBuffer(gl.ARRAY_BUFFER, radBuf);
        gl.bufferSubData(gl.ARRAY_BUFFER, 0, radii.subarray(0, activeCount));
        gl.bindBuffer(gl.ARRAY_BUFFER, colBuf);
        gl.bufferSubData(gl.ARRAY_BUFFER, 0, colors.subarray(0, activeCount*3));
        gl.bindBuffer(gl.ARRAY_BUFFER, alphaBuf);
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, highlightSlotBuf);
  gl.bufferSubData(gl.ARRAY_BUFFER, 0, highlightSlots.subarray(0, activeCount));

        gl.uniform2f(u_resolution, canvas.width, canvas.height);
        // update time uniform
        try { gl.uniform1f(uTime, (now % 100000) / 1000.0); } catch (e) {}
        ...
        highlightedPulsesRef.current.forEach((expiry, id) => {
          if (now > expiry) {
            ...
            if (typeof slot === 'number') updates.push({ slot: slot, r:0, g:0, b:0, a:0 });
            highlightedPulsesRef.current.delete(id);
          }
        });
        // for active highlights, ensure texture contains their color/intensity
        const activeNow = [];
        highlightedPulsesRef.current.forEach((expiry, id) => { activeNow.push(Number(id)); });
        ...
        try { gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, highlightTex); gl.uniform1i(uHighlightTex, 0); } catch (e) {}
  gl.clearColor(0,0,0,0.0);
    ...
    stateRef.current.api = {
      addPulses: (arr) => { replayPulses(arr); },
      getStats: () => ({ fps: statsRef.current.fps, pulsesReceived: statsRef.current.pulsesReceived, pulsesThisSecond: statsRef.current.pulsesThisSecond }),
      ...
      scrubToTimestamp: (timestamp) => {
        try {
          if (ws && ws.readyState === WebSocket.OPEN) {
            ws.send(JSON.stringify({ type: 'request_scrub', timestamp: Number(timestamp) }));
          }
        } catch (e) {}
      },
      resetHighlights: () => { highlightedPulsesRef.current.clear(); },
      ...
      on: (eventName, cb) => {
        ...
        return () => {
          listenersRef.current.set(eventName, (listenersRef.current.get(eventName) || []).filter(x => x !== cb));
        };
      }
    };
    ...
    return () => {
      ...
      // free GL resources
      try { if (gl && highlightTex) gl.deleteTexture(highlightTex); } catch (e) {}
    };
  ...
  useImperativeHandle(ref, () => ({
    ...
    highlightPulses: (ids, duration) => stateRef.current?.api?.highlightPulses(ids, duration) || noop,
    getHighlightedCount: () => stateRef.current?.api?.getHighlightedCount() || 0,
    scrubToTimestamp: (ts) => stateRef.current?.api?.scrubToTimestamp ? stateRef.current.api.scrubToTimestamp(ts) : noop,
