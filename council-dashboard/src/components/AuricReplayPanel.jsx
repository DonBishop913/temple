...
export default function AuricReplayPanel({ wsUrl = (process.env.REACT_APP_AURIC_WS || 'ws://localhost:8765') }) {
  ...
  useEffect(() => {
    ...
    wsRef.current.onerror = (err) => console.warn('AuricReplayPanel: ws error', err)
  ...
  function handleIncomingPulse(pulse) {
    ...
    ripplesRef.current.push({ x, y, start: performance.now(), amplitude: pulse.amplitude, color: pulse.color, duration })
  }
  ...
  useEffect(() => {
    ...
    function draw(now) {
      ...
      ctx.fillText(`pulses: ${pulses.length}  ripples: ${ripplesRef.current.length}`, 8, 16)
  ...
  return (
    <div style={{ padding: 12, background: '#0b1020', color: '#fff', borderRadius: 8 }}>
      <h3 style={{ marginTop: 0 }}>🌌 Auric Replay</h3>
      <div style={{ display: 'flex', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ width: '100%', height: 320, borderRadius: 6, overflow: 'hidden', background: '#071020' }}>
            <canvas ref={canvasRef} style={{ width: '100%', height: '100%', display: 'block' }} />
          </div>
          <div style={{ marginTop: 8, fontSize: 12, color: '#cfe9f1' }}>
            WS: <strong style={{ color: connected ? '#6EE7B7' : '#FFBABA' }}>{connected ? 'connected' : 'disconnected'}</strong>
          </div>
        </div>

        <div style={{ width: 320 }}>
          <div style={{ background: '#081822', padding: 8, borderRadius: 6, maxHeight: 320, overflowY: 'auto' }}>
            <h4 style={{ margin: '6px 0' }}>Recent Pulses</h4>
            <ul style={{ padding: 0, margin: 0, listStyle: 'none' }}>
              {pulses.map((p, i) => (
                <li key={p.id || i} style={{ padding: '6px 8px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer' }}
                    onClick={() => {
                      // dispatch a DOM event to request highlight in WebGL
                      const ev = new CustomEvent('auric-highlight', { detail: { id: p.id, durationMs: 3000 } });
                      window.dispatchEvent(ev);
                    }}>
                  <div style={{ fontSize: 13 }}><strong style={{ color: p.color }}>{(p.amplitude || 0).toFixed(2)}</strong> — <span style={{ color: '#9fb7c2' }}>{p.source}</span></div>
                  <div style={{ fontSize: 11, color: '#7ea0ab' }}>{new Date(p.timestamp).toLocaleString()}</div>
                </li>
              ))}
            </ul>
          </div>
