...
export default function EmotiveGlyphstreamOverlay({
  enabled = true,
  focusNode = null,
  predictive = false,
  onMentorshipFeedback,
  onArchive,
}) {
  ...
  return (
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-40">
      {nodes.map((node) => {
        ...
        const glyph =
          predictive && emotionForecast[node.id]
            ? sentimentToGlyph(emotionForecast[node.id])
            : sentimentToGlyph(sentimentScore);
        const glyphstream = generateGlyphstream({
          heartbeat,
          activity,
          sentimentScore,
        });
