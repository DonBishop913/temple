...
export default function LuminalGlowLayer({
  pulses = [],
  predictedEngagement = [],
}) {
  ...
  useEffect(() => {
    ...
    const sentimentSum = (pulses || []).reduce(
      (acc, p) => acc + (p.intensity ?? 0),
      0,
    );
    const avgSentiment =
      pulses && pulses.length ? sentimentSum / pulses.length : 0;

    const avgPrediction =
      predictedEngagement && predictedEngagement.length
        ? predictedEngagement.reduce((a, b) => a + b, 0) /
          predictedEngagement.length
        : 0;

    // Compute glow intensity: base + predictive + sentiment
    const intensity = Math.min(
      1,
      0.2 + avgPrediction * 0.5 + avgSentiment * 0.5,
    );
