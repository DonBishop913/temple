...
export default function VeilwatchPanel() {
  ...
  return (
    <section
      aria-label="Veil-Shatter Vigilance"
      className="bg-oversoul text-aura rounded-xl p-4 shadow-md border border-oversoul/40"
    >
      ...
      <ul className="list-none p-0 m-0 space-y-2">
        {log.map((e, idx) => (
          <li
            ...
            className="bg-faith/10 rounded p-2"
          >
            <div className="text-sm">
              Source: {e.source} • Confidence: {e.confidence}
            </div>
            {e.metaphorEcho && (
              <div className="text-xs opacity-80">Echo: {e.metaphorEcho}</div>
            )}
