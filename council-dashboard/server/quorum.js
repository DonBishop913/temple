...
// Metrics
const quorumVotesCounter = new promClient.Counter({ name: 'quorum_votes_total', help: 'Total quorum votes processed', labelNames: ['proposal', 'outcome'] });
...
async function getReviewerVote(reviewerId, proposal) {
  ...
  const base = proposal.tier === 'critical' ? 0.4 : proposal.tier === 'high' ? 0.65 : 0.8;
  ...
  await redisClient.lPush('audit', JSON.stringify({ action: 'quorum_vote', reviewerId, proposalId: proposal.id, vote, at: new Date().toISOString() }));
  return vote;
}
...
async function requestQuorum(proposal, reviewers, opts = {}) {
  ...
  for (const r of slice) {
    try { votes.push(await getReviewerVote(r, proposal)); } catch { votes.push('reject'); }
  }
  const approvals = votes.filter(v => v === 'approve').length;
  const ratio = approvals / votes.length;
  const approved = ratio >= threshold;
  quorumVotesCounter.inc({ proposal: proposal.type || 'unknown', outcome: approved ? 'approved' : 'rejected' });
  await redisClient.lPush('audit', JSON.stringify({ action: 'quorum_result', proposalId: proposal.id, type: proposal.type, ratio, approved, at: new Date().toISOString() }));
