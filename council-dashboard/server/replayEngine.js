...
function start() {
  try {
    ...
    wss.on('connection', (ws, req) => {
      ...
      try { ws.send(JSON.stringify({ type: 'replay:hello', at: Date.now(), mode: 'tail' })) } catch {}

      // Bootstrap: send last N entries from timeSync
      (async () => {
        try {
          const entries = await timeSync.fetchRange('-', '+', INITIAL_FETCH_COUNT)
          if (entries && entries.length) {
            // sort by id ascending
            entries.sort((a,b)=> a.id.localeCompare(b.id))
            for (const e of entries) {
              try { ws.send(JSON.stringify({ type: 'replay:event', entry: e })) } catch {}
            }
            client.lastId = entries[entries.length-1].id
          }
        } catch (e) { console.warn('replay bootstrap failed', e && e.message) }
      })()
      ...
      client.pollHandle = setInterval(async () => {
        ...
        try {
          ...
          try { ws.send(JSON.stringify({ type: 'replay:error', message: String(e && e.message) })) } catch {}
        }
      }, POLL_INTERVAL_MS)
            ...
            if (ts) {
              (async () => {
                try {
                  ...
                  for (const e of filtered) try { ws.send(JSON.stringify({ type: 'replay:event', entry: e })) } catch {}
                  if (filtered.length) client.lastId = filtered[filtered.length-1].id
                } catch (ee) { console.warn('seek failed', ee && ee.message) }
              })()
            }
          }
        } catch (e) {
          try { ws.send(JSON.stringify({ type: 'replay:error', message: 'invalid message' })) } catch {}
        }
      })
