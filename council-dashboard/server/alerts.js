...
router.get('/alerts', (req, res) => {
  ...
  const listener = (alert) => {
    try {
      ...
      latencyMetrics.push({ nodeId: alert.nodeId || 'global', emittedAt: alert.timestampEmit })
...
router.post('/emit-node-alert', (req, res) => {
  try {
    ...
    if (!nodeId || !severity) return res.status(400).json({ ok: false, error: 'nodeId and severity required' })
    const now = Date.now()
    const payload = { type: 'nodeAlert', nodeId, severity, message, meta, at: now, timestampEmit: now }
...
router.get('/latency', (req, res) => {
  try {
    ...
    const recent = latencyMetrics.slice(-100).map(m => ({ nodeId: m.nodeId, emittedAt: m.emittedAt, ageMs: now - m.emittedAt }))
