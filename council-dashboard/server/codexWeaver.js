...
async function weaveCodex63({ nodes, forecasts, mentorships, reflections }) {
  const codexContent = nodes.map(node => {
    ...
    if (abtAlignmentCounter) abtAlignmentCounter.inc({ event_type: 'codex_alignment' });
    return {
      nodeId: node.id,
      nodeName: node.name,
      currentEmpathy: node.empathy,
      predictedJoy: forecast ? forecast.peakJoy || forecast.predictedProbability : null,
...
async function autoWeaveCodex63() {
  ...
  try {
    ...
    forecasts = JSON.parse(await redis.get('faithseed:forecast:latest') || '[]');
    mentorships = JSON.parse(await redis.get('mentorship:schedule') || '[]');
    reflections = JSON.parse(await redis.get('mentorship:reflections') || '[]');
  } catch {}
...
router.get('/snapshots', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch snapshots', details: err.message });
  }
});
