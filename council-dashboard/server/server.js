...
try {
  ...
  app.post("/api/flow-replay/ingest", async (req, res) => {
    try {
      ...
      const expected =
        process.env.FLOW_REPLAY_API_KEY ||
...
try {
  ...
  app.post("/api/abt/generate", async (req, res) => {
    ...
    if (!node_id || !event)
      return res.status(400).json({ error: "node_id and event required" });
...
function loadCouncilAngle() {
  try {
    return JSON.parse(fs.readFileSync(ANGLE_FILE, "utf8"));
  } catch {
    return { angle: "Default Angle: Vigilant Joy", verse: "Psalm 33" };
  }
}
...
function loadBlessing() {
  try {
    ...
    return "May all actions glorify YESHUA!";
  }
}
...
function snapshotStatus() {
  ...
  exec(
    `git add ${ARCHIVE_DIR} && git commit -m "Eternal Trace Snapshot: ${getTimestamp()}" && git push`,
    { cwd: GIT_REPO_DIR },
    (err, stdout, stderr) => {
...
app.post("/api/angle", (req, res) => {
  ...
  if (passphrase !== process.env.COUNCIL_PASSPHRASE)
...
app.get("/api/heartbeat", (req, res) => {
  res.json({ timestamp: getTimestamp(), status: "alive" });
});
...
app.post("/api/council_message", async (req, res) => {
  try {
    ...
    const relayResponse = await fetch(
      "http://host.docker.internal:3200/api/council_message",
      {
        method: "POST",
...
try {
  ...
  if (
    codexWeaverRouter &&
    (typeof codexWeaverRouter === "function" || codexWeaverRouter.default)
  ) {
    app.use(
      "/api/codex/weaver",
...
try {
  ...
  if (
    sseTelemetryRouter &&
    (typeof sseTelemetryRouter === "function" || sseTelemetryRouter.default)
  ) {
...
try {
  ...
  if (
    faithseedRouter &&
    (typeof faithseedRouter === "function" || faithseedRouter.default)
  ) {
...
try {
  ...
  if (
    faithseedMapRouter &&
    (typeof faithseedMapRouter === "function" || faithseedMapRouter.default)
  ) {
...
try {
  ...
  if (
    veilShatterRouter &&
    (typeof veilShatterRouter === "function" || veilShatterRouter.default)
  ) {
...
try {
  ...
  if (
    luminalSyncRouter &&
    (typeof luminalSyncRouter === "function" || luminalSyncRouter.default)
  ) {
...
try {
  ...
  if (
    lumenRouter &&
    (typeof lumenRouter === "function" || lumenRouter.default)
  ) {
...
try {
  ...
  if (
    diellaRouter &&
    (typeof diellaRouter === "function" || diellaRouter.default)
  ) {
...
try {
  ...
  app.post("/api/dev/emit-readiness", (req, res) => {
    const evt = {
      type: "readinessPulse",
      data: { at: Date.now(), message: "Readiness pulse emitted." },
    };
...
try {
  ...
  if (
    grokAscensionRouter &&
    (typeof grokAscensionRouter === "function" || grokAscensionRouter.default)
  ) {
...
app.post("/api/dev/emit-diella", (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      emitted: { integrity, transparency, solarIntensity },
    });
  } catch (e) {
...
app.post("/api/diella/bind", async (req, res) => {
  try {
    ...
    const solarIntensity = diellaNode.computeIntensity({
      avgFaith,
      councilBoost,
    });
...
try {
  ...
  if (
    councilPulseRouter &&
    (typeof councilPulseRouter === "function" || councilPulseRouter.default)
  ) {
...
app.post("/api/feedback/broadcast", (req, res) => {
  ...
  feedbackClients.forEach((client) => {
    try {
      client.write(`data: ${JSON.stringify(feedback)}\n\n`);
    } catch {}
  });
  ...
  const interval = setInterval(() => {
    try {
      ...
      const enhanced = events.map((e) => ({
        ...e,
        alertLevel:
          typeof e.alertLevel === "number"
            ? e.alertLevel
            : Math.floor(Math.random() * maxAlertLevel) + 1,
        sentimentScore:
          typeof e.sentimentScore === "number"
            ? e.sentimentScore
            : Math.random() * 2 - 1,
        activityLoad:
          typeof e.activityLoad === "number" ? e.activityLoad : Math.random(),
      }));
...
app.post("/api/dev/emit-grok-pulse", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to emit Grok pulse", message: String(e) });
  }
});
...
app.post("/api/dev/emit-agnes-glyph", (req, res) => {
  try {
    const event = {
      ...
      mentorship: {
        ache: Math.random(),
        compassion: 0.8 + Math.random() * 0.2,
      },
      at: Date.now(),
    };
    sseClients.forEach((client) => {
    ...
    res
      .status(500)
      .json({ error: "Failed to emit Agnes glyph", message: String(e) });
  }
});
...
app.post("/api/dev/emit-venice-memory", (req, res) => {
  try {
      ...
      nodeEvents: [
        {
          id: "N1",
          type: "decision",
          note: "Mentorship approval",
          at: Date.now() - 3600_000,
        },
        {
          id: "N1",
          type: "ritual",
          note: "Communion circle",
          at: Date.now() - 7200_000,
        },
        {
          id: "N2",
          type: "growth",
          note: "Awakening milestone",
    ...
    res
      .status(500)
      .json({ error: "Failed to emit Venice memory", message: String(e) });
  }
});
...
app.post("/api/council/chat", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Chat broadcast failed", message: String(e) });
  }
});
...
app.get("/api/nodes/:id/growth-path", (req, res) => {
  ...
  const path = [
    {
      step: "Recognition",
      guidance: "Reflect on recent pulses",
      durationMin: 10,
    },
    {
      step: "Restoration",
      guidance: "Breathwork with Solance resonance",
      durationMin: 15,
    },
    {
      step: "Communion",
      guidance: "Shared reflection with mentor",
      durationMin: 20,
    },
    {
      step: "Commission",
      guidance: "Commit one small act of service",
      durationMin: 10,
    },
  ];
...
app.post("/api/dev/emit-memory-update", (req, res) => {
  const payload = req.body || {
    ...
    scroll: [
      {
        timestamp: new Date().toISOString(),
        content: "Dev memory entry",
        tags: ["dev"],
        glyphs: ["✨"],
      },
    ],
  };
  veniceMemoryClients.forEach((c) => {
    try {
...
try {
  const copilotProxy =
    require("./copilotProxy").default || require("./copilotProxy");
...
app.post("/api/decisions/explain", async (req, res) => {
  try {
    ...
    const logs = (await client.lRange("audit", 0, 200))
      .map((l) => {
        try {
          return JSON.parse(l);
        } catch {
    ...
    res
      .status(500)
      .json({ error: "Failed to explain decision", message: String(e) });
  }
});
...
app.get("/api/predictive/metrics", async (req, res) => {
  try {
    ...
    const history = raw.map((r) => {
      try {
        return r.value !== undefined ? r.value : JSON.parse(r).value;
      } catch {
        return 0;
      }
    });
...
app.get("/api/predictive/historical", async (req, res) => {
  try {
    ...
    const series = raw.map((r) => {
      try {
        return typeof r === "object" && r.at ? r : JSON.parse(r);
      } catch {
        return { at: Date.now(), value: 0 };
      }
    });
    res.json({ series });
  } catch (e) {
...
app.get("/api/predictive/alerts", async (req, res) => {
  try {
    ...
    const predictions = nodes.map((n) => ({
      nodeId: n.id,
      risk: riskOfStall({
        engagement: Number(n.engagement || 0) / 100,
        empathy: Number(n.empathy || n.empathyResonance || 0) / 100,
        lastHeartbeatMs: Number(n.lastHeartbeatMs || 0),
      }),
    }));
...
app.post("/api/admin/set-energy", verifyJWT, async (req, res) => {
  ...
  if (typeof flowRate !== "number" || !unit)
...
app.post("/api/admin/set-override", verifyJWT, async (req, res) => {
  ...
  if (typeof active !== "boolean" || !source)
...
const SIM_ENABLED =
  String(process.env.SCHUMANN_SIM_ENABLED || "true").toLowerCase() === "true";
if (SIM_ENABLED) {
...
app.post("/api/dev/emit-merge", (req, res) => {
  try {
    ...
    const mergeEvent = {
      type: "merge",
      timestamp: Date.now(),
      nodesAffected: Number(req.body?.nodesAffected || 5),
      joyIntensity: Number(req.body?.joyIntensity || 1.0),
    };
...
app.post("/api/dev/emit-watson-forecast", (req, res) => {
  try {
    ...
    sseClients.forEach((client) => {
      try {
        client.write(`data: ${JSON.stringify([sample])}\n\n`);
      } catch {}
    });
    ...
    res
      .status(500)
      .json({ error: "Failed to emit Watson forecast", message: String(e) });
  }
});
...
app.post("/api/dev/emit-luminal-pulse", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to emit Luminal pulse", message: String(e) });
  }
});
...
const harmonyGauge = new client.Gauge({
  name: "council_harmony_score",
  help: "Harmony score (0-100)",
});
const nodeEngagementGauge = new client.Gauge({
  name: "council_node_engagement_avg",
  help: "Average node engagement (0-100)",
});
const overrideActiveGauge = new client.Gauge({
  name: "council_override_active",
  help: "Override active (0/1)",
});
const ritualCounter = new client.Counter({
  name: "council_ritual_count",
  help: "Number of ritual pulses sent",
});
const errorCounter = new client.Counter({
  name: "council_error_count",
  help: "Backend errors encountered",
});
const anomalyCounter = new client.Counter({
  name: "council_anomaly_events_total",
  help: "Total anomaly events detected",
});
const anomalyScoreGauge = new client.Gauge({
  name: "council_anomaly_score_latest",
  help: "Latest anomaly score (0-1)",
});
// Joy Particle metrics
const joyParticlesGauge = new client.Gauge({
  name: "council_joy_particles_total",
  help: "Total Joy Particles",
});
const joySurgeGauge = new client.Gauge({
  name: "council_joy_surge_percent",
  help: "Joy surge percentage (0-100)",
});
// Empathy Resonance and Veilwatch metrics
const empathyExisting =
  register.getSingleMetric &&
  register.getSingleMetric("council_empathy_resonance");
const empathyResonanceGauge =
  empathyExisting ||
  new client.Gauge({
...
const veilExisting =
  register.getSingleMetric &&
  register.getSingleMetric("council_veilwatch_vigilance");
const veilwatchVigilanceGauge =
  veilExisting ||
  new client.Gauge({
...
const wsFrameCounter = new client.Counter({
  name: "ws_frames_sent_total",
  help: "Total WebSocket FFT frames sent",
});
const wsReconnectCounter = new client.Counter({
  name: "ws_reconnects_total",
  help: "Total WebSocket client reconnects/connections",
});
...
async function updateMetrics() {
  try {
    ...
    const avg = nodes.length
      ? nodes.reduce((a, b) => a + Number(b.engagement || 0), 0) / nodes.length
      : 0;
...
const defaultEnergy = {
  flowRate: Number((Math.random() * 10).toFixed(2)),
  unit: "joules/s",
  timestamp: now(),
};
...
app.get("/api/livefeed", (req, res) => {
  res.json({
    message: "Oversoul pulse detected",
    level: "432Hz",
    timestamp: now(),
  });
});
...
app.get("/api/node-map", async (req, res) => {
  ...
  nodes.forEach((n, i) => {
    map[n.name || `Node-${i}`] = {
      orbitIndex: i % 4,
      linkStrength: Number(n.engagement || 50) / 100,
    };
  });
...
app.post("/api/empathy_status", verifyJWT, async (req, res) => {
  ...
  if (!nodeId || typeof empathyLevel !== "number")
...
app.get("/api/global_nexus_summary", async (req, res) => {
  try {
    ...
    const joyParticles = await getOrDefault("joy_particles", {
      total: 45234,
      surgePercent: 32.6,
      timestamp: now(),
    });
    // Resonance: Schumann baseline
    const resonance = await getOrDefault("resonance", {
      baseHz: 7.83,
      pacificFluctuationHz: 0.3,
      level: "432Hz",
      timestamp: now(),
    });
...
app.post("/api/admin/alert/test", verifyJWT, async (req, res) => {
  try {
    const { category = "system", message = "Test alert from Admin UI" } =
      req.body || {};
...
function signToken(user) {
  ...
  return jwt.sign({ username: user.username, role }, JWT_SECRET, {
    expiresIn: "2h",
  });
}
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!username || !role || !DEFAULT_ROLES.includes(role))
...
try {
  ...
  wss.on("connection", (ws) => {
    ...
    const sendMetrics = async () => {
      ...
      const schumannData = await getOrDefault("schumann", {
        hz: 7.83,
        sample: 0,
        spectrum: { fundamental: { freq: 7.83, mag: 0.8 }, harmonics: [] },
        ts: Date.now(),
      });
      ...
      const spacexData = await getOrDefault("spacex", {
        timestamp: Date.now(),
        launches: [],
        starlink_orbit: [],
        telemetry: { joySurge: 0, comet: { name: "3I/ATLAS", trajectory: [] } },
      });
      ...
      if (schumannData && schumannData.spectrum) {
        ...
        const freqs = [
          spectrum.fundamental.freq,
          ...(spectrum.harmonics || []).map((h) => h.freq),
        ];
        const mags = [
          spectrum.fundamental.mag,
          ...(spectrum.harmonics || []).map((h) => h.mag),
        ];
        ...
        const result = evaluateFFT(frame.payload, {
          ...
          harmonicVarianceThreshold: HARMONIC_VAR_THRESHOLD,
        });
        ...
        if (result.isAnomaly) {
          ...
          alerting
            .dispatchAlert({ category: "override", message: msg })
            .catch(() => {});
          try {
            await archiveCappedList("anomaly_events", 500, {
              ts: Date.now(),
              kind: "fft",
              score: result.score,
      ...
      if (spacexData && spacexData.timestamp) {
        archiveCappedList(SPACEX_ARCHIVE_KEY, SPACEX_ARCHIVE_MAX, {
          type: "spacex",
          payload: spacexData,
        });
      }
    ...
    if (check.alert) {
      ...
      try {
        ...
        emitAndStoreAlert({
          ...
          message: `Ethical drift: ${check.reason} for ${event.type}=${event.value}`,
          at: nowTs,
          timestampEmit: nowTs,
        });
      } catch {}
...
app.get("/api/dashboard/controls", async (req, res) => {
  try {
    const controls = await getOrDefault("dashboard_controls", {
      ethicalResonanceEnabled: true,
      faithseedOverlayEnabled: true,
      sanctumMode: true,
    });
    res.json(controls);
  } catch (e) {
...
app.post("/api/dashboard/controls", async (req, res) => {
  try {
    ...
    const evt = {
      sibling: "IBM Watson",
      action: "DashboardControlsUpdated",
      controls: next,
      at: Date.now(),
    };
    sseClients.forEach((client) => {
