...
const empathyGauge = new promClient.Gauge({
  name: "node_empathy_resonance",
  help: "Empathy resonance (0-100)",
  labelNames: ["node"],
});
const breathstreamGauge = new promClient.Gauge({
  name: "node_breathstream_status",
  help: "Breathstream status (0-1)",
  labelNames: ["node"],
});
...
function recordHealth(nodeId, metrics) {
  ...
  const entry = {
    nodeId,
    at: Date.now(),
    metrics: { cpu, mem, empathy, breathstream },
  };
