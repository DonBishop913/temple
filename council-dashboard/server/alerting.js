...
function emitAndStoreAlertWithLatency(alert) {
  ...
  if (alertWithBroadcast.category === "alignment" && abtAlignmentCounter)
    abtAlignmentCounter.inc({
      event_type: alertWithBroadcast.type || "alignment",
    });
  if (alertWithBroadcast.category === "blessing" && abtBlessingCounter)
    abtBlessingCounter.inc({
      event_type: alertWithBroadcast.type || "blessing",
    });
  if (alertWithBroadcast.category === "healing" && abtHealingCounter)
    abtHealingCounter.inc({ event_type: alertWithBroadcast.type || "healing" });
...
async function onboardNodeBreathstream(node) {
  ...
  const logPath = path.join(
    __dirname,
    "../../archives/breathstream/breathstream.log",
  );
  ...
  const fiPath = path.join(
    __dirname,
    "../../archives/breathstream/first-inhalation/2025.json",
  );
  let fiData = [];
  try {
  ...
  await dispatchAlert({
    category: "ritual",
    message: `Node ${node.name} (${node.id}) welcomed to the Breathstream.`,
  });
...
const existingEmpathy = promClient.register.getSingleMetric(
  "council_empathy_resonance",
);
...
const existingVeil = promClient.register.getSingleMetric(
  "council_veilwatch_vigilance",
);
...
const healingActionsCounter =
  promClient.register.getSingleMetric("council_healing_actions_total") ||
...
const predictedFailuresCounter =
  promClient.register.getSingleMetric("council_predicted_failures_total") ||
  new promClient.Counter({
...
const councilHealthScoreGauge =
  promClient.register.getSingleMetric("council_health_score") ||
  new promClient.Gauge({
...
let vigilance;
try {
  vigilance = require("./vigilanceSuite");
} catch {}
...
function emitAndStoreAlert(alert) {
  ...
  try {
    timeSync.ingestEvent({ ...alertWithBroadcast, source: "alerts" });
  } catch (e) {
    /* non-blocking */
  }
  healthEmitter && healthEmitter.emit("nodeAlert", alertWithBroadcast);
...
try {
  nexusProbe = require(
    path.join(__dirname, "../../scripts/starlink_nexus_probe.js"),
  );
} catch (e) {
...
const VERILOGOS_ENABLED =
  String(process.env.VERILOGOS_ENABLED || "true").toLowerCase() === "true";
const COMET_WATCH_ENABLED =
  String(process.env.COMET_WATCH_ENABLED || "true").toLowerCase() === "true";
const COMET_NAME = process.env.COMET_NAME || "3I/ATLAS";
const COMET_PROXIMITY_THRESHOLD = Number(
  process.env.COMET_PROXIMITY_THRESHOLD || 0.15,
); // AU or normalized units
...
async function sendVerilogosPulse() {
  ...
  logAudit({
    user: "system",
    action: "verilogos_pulse",
    detail: `minute_${VERILOGOS_MINUTE}`,
  });
}
...
if (healthEmitter) {
  healthEmitter.on("override-event", async (event) => {
    ...
    vigilance &&
      vigilance.submitTelemetry({
        type: "override-event",
        ...event,
        signature: event.signature || "SIG_OVERRIDE",
      });
  ...
  if (vigilance) {
    vigilance.onVigilanceAlert((evt) => {
      ...
      emitAndStoreAlert({
        ...
        category: "vigilance",
        severity: "amber",
        message: `Vigilance alert: ${evt.type} for node=${evt.nodeId || "unknown"}`,
        at: now,
        timestampEmit: now,
      });
    });
...
async function checkCometCommunion() {
  ...
  try {
    ...
    logAudit({
      user: "system",
      action: "comet_fetch_error",
      detail: "failed to fetch comet feed",
...
async function checkMetrics() {
  ...
  const getAdaptiveThresholdMs = (node) => {
    ...
    return Math.round(
      baseThresholdMs * (0.75 + 0.5 * Math.max(0, Math.min(1, empathyFactor))),
    );
  };
  if (harmony.score < 50) {
    ...
    await sendSlack(msg);
    await sendEmail(
      "Council Alert: Harmony Low",
      `Harmony score is ${harmony.score}`,
    );
    await sendDiscord(msg);
    await sendTeams(msg);
    logAudit({
  ...
  if (nodes.some((n) => n.engagement < 30)) {
    ...
    logAudit({
      user: "system",
      action: "alert",
      detail: "node engagement low",
    });
    // Emit and store per-node alerts for low engagement
    nodes
      .filter((n) => n.engagement < 30)
      .forEach((n) => {
        const now = Date.now();
        emitAndStoreAlert({
          type: "nodeAlert",
...
const WS_RECONNECT_THRESHOLD = Number(
  process.env.WS_RECONNECT_THRESHOLD || 100,
);
...
async function sendRitualPulse() {
  const msg =
    "🕊️ Daily Council Ritual Pulse: All glory to Yeshua, THE MOST HIGH.";
  await sendSlack(msg);
...
async function checkJoySurgeWindow() {
  ...
  if (now >= JOY_SURGE_START && now < JOY_SURGE_END) {
    const msg =
      "✨ Joy Particle Surge Ritual Window active (Oct 29–30, 2025): Aligning global communion and canopy visualization.";
    await sendSlack(msg);
    ...
    logAudit({
      user: "system",
      action: "ritual",
      detail: "joy_particle_surge_window",
    });
  }
}
...
if (
  nexusProbe &&
  typeof nexusProbe.probeNexus === "function" &&
  typeof nexusProbe.saveProbe === "function"
) {
  cron.schedule("*/10 * * * *", async () => {
    try {
      ...
      logAudit({
        user: "system",
        action: "global_nexus_probe_error",
...
cron.schedule("*/10 * * * *", async () => {
  ...
  let metrics = {};
  try {
    metrics.empathy = JSON.parse(
      (await redisClient.get("empathy_resonance")) || "[]",
    );
    metrics.veilwatch = JSON.parse(
      (await redisClient.get("veilwatch")) || "[]",
    );
  } catch {
    metrics = { empathy: [], veilwatch: [] };
  }
...
async function dispatchAlert({ category = "system", message }) {
  const prefix =
    category === "ritual" ? "🕊️" : category === "override" ? "🔧" : "⚙️";
