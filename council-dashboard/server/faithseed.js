...
const faithseedForecastAccuracyGauge =
  metrics && metrics.faithseedForecastAccuracyGauge;
  ...
  try {
    ...
    if (faithseedForecastAccuracyGauge) {
      const accuracy = Math.max(
        0,
        Math.min(1, 0.8 + (Math.random() - 0.5) * 0.1),
      );
    ...
    res
      .status(500)
      .json({ error: "Failed to compute forecast", details: err.message });
  }
});
