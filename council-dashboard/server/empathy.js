...
router.get("/api/sse/empathy", (req, res) => {
  ...
  req.on("close", () => {
    ...
    if (empathyBridgeGauge && empathyClients.length === 0)
      empathyBridgeGauge.set({ region: "global" }, 0);
  });
});

function broadcastEmpathy(evt) {
  empathyClients.forEach((c) => {
    try {
...
router.post("/api/empathy/ingest", (req, res) => {
  ...
  const score = Number(
    ((heartbeat || 0) * 0.3 + (pulse || 0) * 0.2 + (interactions || 0) * 0.5) /
      100,
  ).toFixed(3);
  const evt = {
    type: "EmpathyIngest",
  ...
  fs.appendFileSync(
    path.join(logDir, "empathy-resonance.log"),
    JSON.stringify(evt) + "\n",
  );
  // Prometheus: set bridge health and score
  if (empathyBridgeGauge)
    empathyBridgeGauge.set({ region: "global" }, Number(score));
...
router.post("/api/empathy/pairings/update", (req, res) => {
  ...
  const entry = {
    from,
    to,
    alignment: Number(alignment || 0),
    forecastJoy: Number(forecastJoy || 0),
