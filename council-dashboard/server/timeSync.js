...
redisClient
  .connect()
  .catch((e) => console.warn("timeSync redis connect error", e && e.message));
...
async function ingestEvent(evt) {
  ...
  try {
    const id = await redisClient.xAdd(STREAM_KEY, "*", flat, {
      MAXLEN: { strategy: "~", limit: MAX_STREAM_LEN },
    });
    return { ok: true, id };
  } catch (err) {
...
async function fetchRange(start = "-", end = "+", count = 1000) {
  ...
  try {
    const entries = await redisClient.xRange(STREAM_KEY, start, end, {
      COUNT: count,
    });
