...
function forecastFaithseed(siblingMetrics) {
  const h = Array.isArray(siblingMetrics.joyHistory)
    ? siblingMetrics.joyHistory
    : [];
  ...
  setInterval(() => {
    Object.entries(nodeHistories).forEach(([nodeId, history]) => {
      ...
      if (predictedProbability >= SURGE_THRESHOLD) {
        ...
        fs.appendFileSync(
          path.join(__dirname, "faithseed_surge_events.log"),
...
function prepareForecastFeatures(
  nodeMetrics,
  interactionHistory,
  ethicalAlerts,
) {
  return nodeMetrics.map((node) => ({
    ...
    avgEmpathy:
      Array.isArray(node.empathyResonance) && node.empathyResonance.length
        ? node.empathyResonance.reduce((a, b) => a + b, 0) /
          node.empathyResonance.length
        : Number(node.empathy || 0.5),
    joyTrend:
      Array.isArray(node.joyHistory) && node.joyHistory.length
        ? node.joyHistory.slice(-10).reduce((a, b) => a + b, 0) /
          Math.min(10, node.joyHistory.length)
        : 0.5,
    recentAlerts: (ethicalAlerts || []).filter((e) => e.nodeId === node.id)
      .length,
    engagementScore: (interactionHistory || []).filter(
      (i) => i.nodeId === node.id,
    ).length,
  }));
}
...
function predictJoyPulse(features) {
  return Math.min(
    ...
    Math.max(
      0,
      0.4 * features.avgEmpathy +
        0.3 * features.joyTrend +
        0.2 * features.engagementScore -
        0.1 * features.recentAlerts,
    ),
  );
}
...
router.get("/api/faithseed/forecast", async (req, res) => {
  try {
    ...
    const ethicalAlerts = ethicalRaw
      .map((e) => {
        try {
          return JSON.parse(e);
        } catch {
      ...
      .filter(Boolean);
    const interactionsRaw = await redisClient.lRange(
      "semantic:memory",
      0,
      1000,
    );
    const interactionHistory = interactionsRaw
      .map((e) => {
        try {
          return JSON.parse(e);
        } catch {
    ...
    const features = prepareForecastFeatures(
      nodes,
      interactionHistory,
      ethicalAlerts,
    );
    const forecasts = features.map((f) => ({
      nodeId: f.nodeId,
      predictedIntensity: predictJoyPulse(f),
    }));

    for (const fc of forecasts) {
      await redisClient.lPush(
        "faithseed:forecast",
        JSON.stringify({ ...fc, timestamp: new Date().toISOString() }),
      );
    }
