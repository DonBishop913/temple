...
router.post('/register-member', (req, res) => {
  try {
    ...
    if (!memberName || !channel) return res.status(400).json({ error: 'memberName and channel required' });
    const members = JSON.parse(fs.readFileSync(configFile));
    if (!members.find(m => m.name === memberName)) {
      members.push({ name: memberName, channel, joinedAt: new Date().toISOString() });
