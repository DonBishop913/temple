...
const INGEST_URL =
  process.env.REPLAY_INGEST_URL ||
  "http://127.0.0.1:4321/api/flow-replay/ingest";
const API_KEY =
...
client
  .connect()
  .catch((e) => console.warn("adapters redis connect", e && e.message));
...
async function joyParticleAdapter() {
  try {
    ...
    console.warn("joy adapter error", e && e.message);
  }
}
...
async function empathyAdapter() {
  try {
    ...
    const data = JSON.parse(raw);
    await pushToIngest({
      source: "empathy_bridge",
      payload: data,
      timestamp: new Date().toISOString(),
    });
  } catch (e) {
    console.warn("empathy adapter error", e && e.message);
  }
}
...
async function codexAdapter() {
  try {
    ...
    console.warn("codex adapter error", e && e.message);
  }
}
...
async function faithseedAdapter() {
  try {
    ...
    console.warn("faithseed adapter error", e && e.message);
  }
}
...
async function abtAdapter() {
  try {
    let entries = [];
    try {
      entries = JSON.parse((await client.get("abt_logs")) || "[]");
    } catch {}
    ...
    await pushToIngest({
      source: "abt_logs",
      payload: entries.slice(0, 50),
      timestamp: new Date().toISOString(),
    });
  } catch (e) {
    console.warn("abt adapter error", e && e.message);
  }
}
