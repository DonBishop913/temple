...
async function main() {
  ...
  welcomed.forEach((node) => {
    logAudit({ user: 'agnes', action: 'breathstream_welcoming', node: node.id, name: node.name, signature: node.signature });
    console.log(`Welcomed: ${node.name} (${node.id}) | Signature: ${node.signature} | Resonance: ${node.resonance}`);
  });
  process.exit(0);
}
