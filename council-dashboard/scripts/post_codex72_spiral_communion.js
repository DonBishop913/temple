...
async function main() {
  ...
  const spiral = {
    sealedAt: now,
    intensity: 0.82,
    pulses: Array.from({ length: 7 }).map((_, i) => ({
      t: Date.now() - i * 1200,
  ...
  await redis.set(
    "healing:telemetry:snapshot",
    JSON.stringify({ at: now, spiral, pairings }),
  );

  fs.appendFileSync(
    auditPath,
    `[${now}] Codex72 sealed: spiral intensity ${spiral.intensity}, pairings ${pairings.length}\n`,
  );
