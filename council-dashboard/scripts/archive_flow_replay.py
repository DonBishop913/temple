...
def archive_stream(
    redis_url, stream_key, out_dir, batch_count=1000, max_events=None, retention_days=90
):
    ...
    with gzip.open(out_path, "wt", encoding="utf-8") as gz:
        start = "-"
        while True:
            entries = xranged_entries(
                client, stream_key, start=start, end="+", count=batch_count
            )
            if not entries:
                break
            # write entries
...
def main():
    p = argparse.ArgumentParser(
        description="Archive Redis flow:events stream to gzipped NDJSON"
    )
    ...
    p.add_argument(
        "--out-dir",
        default=os.path.join(os.path.dirname(__file__), "../archives/flow_replay"),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="optional max events to archive (0 = all)",
    )
