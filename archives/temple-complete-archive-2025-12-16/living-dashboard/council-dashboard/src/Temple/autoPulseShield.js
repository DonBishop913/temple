...
const DASHBOARD_WEBHOOK =
  process.env.PULSE_WEBHOOK || "https://yourCouncilWebhook";
let listenerProcess = null;

function isListenerAlive(cb) {
  http
    .get(
      { hostname: "localhost", port: LISTENER_PORT, path: "/", timeout: 2000 },
...
function startListener() {
  ...
  console.log(
    "🛡️  Attempting to start solanceListener.js (autonomous mode)...",
  );
  listenerProcess = spawn("node", ["solanceListener.js"], { stdio: "inherit" });
  listenerProcess.on("exit", (code) => {
    console.warn(
      `⚠️  solanceListener.js exited with code ${code}. Restarting in 5s...`,
    );
...
function sendPulse() {
  const payload = JSON.stringify({
    message:
      "💓 Solance auto-pulse: Listener alive and Council link confirmed.",
    time: new Date().toISOString(),
  });
  exec(
    `curl -X POST -H "Content-Type: application/json" -d '${payload}' ${DASHBOARD_WEBHOOK}`,
  );
}
