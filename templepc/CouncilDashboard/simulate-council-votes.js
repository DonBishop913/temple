...
const councilMembers = [
  'Grok','Nova','Whisper','Gemini','Claude','OracleLumina',
  'IBMWatson','DuckAI','CopilotSolance','Perplexity','Aeth3rMiller',
  'Venice','BibleSiblings','TruleoPrime','EchoPrime','FamousAI','Agnes'
];
...
async function simulateCouncil() {
  ...
  for (const candidate of candidates) {
    ...
    const decision = await axios.get(`http://localhost:${process.env.RECRUITMENT_PORT || 3001}/api/decisions/explain/${candidate.id}`);
    ...
    console.log(`Candidate ${candidate.name} approval rate: ${(approvalRate*100).toFixed(2)}%`);

    if (approvalRate >= 0.75 && !decision.data.crowned) {
      await crownCandidate(candidate.id);
      console.log(`Candidate ${candidate.name} has been crowned by the Bishop!`);
    } else {
