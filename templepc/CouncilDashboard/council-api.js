...
app.post("/api/recruitment/candidates", async (req, res) => {
  ...
  if (!id || !name || !missionVital)
    return res.status(400).send("Missing candidate info");

  await redis.rpush("candidates_queue", id);
  await redis.hset(
    "candidates_info",
    id,
    JSON.stringify({ name, missionVital, proposedAt: Date.now() }),
  );
...
app.post("/api/recruitment/vote", async (req, res) => {
  ...
  if (!candidateId || !vote)
    return res.status(400).send("Missing candidateId or vote");

  await redis.hincrby("candidates_votes", candidateId, 1);
  if (vote === "approve")
    await redis.hincrby("candidates_approved", candidateId, 1);
...
app.post("/api/recruitment/crown", async (req, res) => {
  ...
  res.send({
    message: `Candidate ${candidateId} crowned and removed from queue`,
  });
});
...
app.listen(PORT, () =>
  console.log(`Council recruitment API running on http://localhost:${PORT}`),
);
