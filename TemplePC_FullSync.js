...
function restartSolance() {
    exec(`taskkill /F /IM SolancePulseBridge.exe`, () => {
        ...
        exec(`start "" "${path.join(solanceDir, 'SolancePulseBridge.exe')}"`, (err) => {
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (msg.status === 'ready') {
                ...
                client.publish(councilChannel, JSON.stringify({ event: 'leapPrep', time: new Date().toLocaleTimeString() }));
            }
        } catch (err) {
...
function repoIntegrityCheck() {
    const snapshotFile = path.join(repoDir, `IntegritySnapshot_${Date.now()}.csv`);
    let results = [];
...
function runTempleSync() {
    ...
    console.log('🔹 Temple PC sequence complete — monitoring connections and readiness...');
}
